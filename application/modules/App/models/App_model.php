...
class App_model extends CI_Model {
   ...
   function get_title_topics_des($title_id)
    ...
    public function get_order_list($filter = array(), $order = null, $dir = null, $count = false) 
    {
        ...
        $this->db->select('orders.*, programs.name AS programName , user_meta.meta_value as school_name, CONCAT(orders_created.first_name, " ", orders_created.last_name) as created_by_name, CONCAT(orders_updated.first_name, " ", orders_updated.last_name) as updated_by_name, titles.name as title_name, (SELECT SUM(total_hours) FROM order_schedules WHERE order_schedules.order_id = orders.id'.$scheduleByPresenter.') AS total_hours_scheduled, (SELECT MAX(start_date) FROM order_schedules WHERE order_schedules.order_id = orders.id) AS last_day_scheduled'.$selectQuery);
        $this->db->from('orders');
        $this->db->join('titles', 'orders.title_id = titles.id');
        $this->db->join('programs', 'orders.program_id = programs.id','left');
        $this->db->join('users AS schools', 'orders.school_id = schools.id');
        $this->db->join('user_meta', "user_meta.user_id = schools.id AND user_meta.meta_key = 'school_name'", "left outer");
        //$this->db->join('users AS presenters', 'orders.presenter_id = presenters.id');
        $this->db->join('users AS orders_created', 'orders.created_by = orders_created.id', 'left');
    ...
    **/
    ...
    public function get_download_status($order_id, $presenter_id, $session_from, $session_to){
       ...
       $this->db->where('session_from', $session_from);
       $this->db->where('session_to', $session_to);
       $this->db->where('is_public_title !=', 'checked'); // 24-05-2023
       $query = $this->db->get();
        // echo $this->db->last_query(); die();
        if($query->num_rows() > 0 ){
            return true;
    ...
    public function get_status_createInvoice_by_inv_id_range($order_id, $presenter_id, $inv_id,$firstDate,$endDate,$stat,$schedules_id){
        ...
        $this->db->where('DATE_FORMAT(order_schedules.start_date, "%Y-%m-%d") >= ', $firstDate);
        $this->db->where('DATE_FORMAT(order_schedules.start_date, "%Y-%m-%d") <= ', $endDate);
        if($stat == 'checked'){ // 23-05-2023
            $this->db->where('end_date <', date('Y-m-d H:i:s')); // 23-05-2023
        }
        $query = $this->db->get();
        // echo $this->db->last_query(); echo '</br>'; echo '</br>';
        // // die();
    ...
    public function get_dates($order_id){
    ...
    // below methods added to fetch programs by mukta for phase 6 work
    function get_program_list($filter = array()) {
    	$this->db->select('programs.*, CONCAT(titles_created.first_name, " ", titles_created.last_name) as created_by_name, CONCAT(titles_updated.first_name, " ", titles_updated.last_name) as updated_by_name');
		$this->db->from('programs');
		$this->db->join('users AS titles_created', 'programs.created_by = titles_created.id', 'left');
		$this->db->join('users AS titles_updated', 'programs.updated_by = titles_updated.id', 'left');
    	if (isset($filter['status']) && $filter['status'] <> "") {
   			$this->db->where('programs.status', $filter['status']);
   		}
		if (isset($filter['deleted']) && $filter['deleted'] !== "") {
            $this->db->where('programs.is_deleted', $filter['deleted']);
   		}
   		// $this->db->order_by('name ASC');
    ...
    function get_program_details($id=null) {
    	$id = (int) $id;
        if($id){
    	$this->db->select('programs.*, CONCAT(grades_created.first_name, " ", grades_created.last_name) as created_by_name, CONCAT(grades_updated.first_name, " ", grades_updated.last_name) as updated_by_name');
		$this->db->from('programs');
		$this->db->join('users AS grades_created', 'programs.created_by = grades_created.id', 'left');
       ...
       public function get_program_category_details($id='') {
        $this->db->select('programs.*, orders.id AS order_id');
        $this->db->from('programs');
        $this->db->join('orders', 'orders.program_id = programs.id', 'left');
        if ($id != '') {
            $this->db->where('programs.id', $id);
        }
        $query = $this->db->get();
    ...
    //below is for participant type's necessary information

    function get_participant_list($filter = array()) 
    {
    	$this->db->select('participants.*, CONCAT(titles_created.first_name, " ", titles_created.last_name) as created_by_name, CONCAT(titles_updated.first_name, " ", titles_updated.last_name) as updated_by_name');
		$this->db->from('participants');
		$this->db->join('users AS titles_created', 'participants.created_by = titles_created.id', 'left');
		$this->db->join('users AS titles_updated', 'participants.updated_by = titles_updated.id', 'left');
    	if (isset($filter['status']) && $filter['status'] <> "") {
   			$this->db->where('participants.status', $filter['status']);
   		}
		if (isset($filter['deleted']) && $filter['deleted'] !== "") {
            $this->db->where('participants.is_deleted', $filter['deleted']);
   		}
   		// $this->db->order_by('name ASC');
		$this->db->group_by("participants.id");
   		$query = $this->db->get();
   		//$query = $this->db->get();
   		//echo $this->db->last_query()."<br>";
   		return $query->result();
    ...
    public function get_participant_category_details($id='') {
        $this->db->select('participants.*');
        $this->db->from('participants');
        // $this->db->join('orders', 'orders.program_id = participants.id', 'left');
        if ($id != '') {
    ...
    function get_participant_details($id=null) {
    	$id = (int) $id;
        if($id){
    	$this->db->select('programs.*, CONCAT(grades_created.first_name, " ", grades_created.last_name) as created_by_name, CONCAT(grades_updated.first_name, " ", grades_updated.last_name) as updated_by_name');
		$this->db->from('programs');
    ...
    //     // if ($id != '') {
    //     //     $this->db->where('participants.id', $id);
    //     //     $this->db->where('participants.is_deleted', 0);
    //     // }
    //     $this->db->where('participants.is_deleted', 0);
    //     $query = $this->db->get();
    //     return $query->result();
    ...
    // from aditi 15-02-2024

    public function get_servicetype_list($filter = array(), $order = null, $dir = null, $count = false) {
        $this->db->select('service_type.*, CONCAT(service_type_created.first_name, " ", service_type_created.last_name) as created_by_name, CONCAT(service_type_updated.first_name, " ", service_type_updated.last_name) as updated_by_name');
        $this->db->from('service_type');
        $this->db->join('users AS service_type_created', 'service_type.created_by = service_type_created.id', 'left');
        $this->db->join('users AS service_type_updated', 'service_type.updated_by = service_type_updated.id', 'left');
        if (isset($filter['question_group']) && $filter['question_group'] !== "") {
            $this->db->where('service_type.question_group', $filter['question_group']);
        }
        if (isset($filter['deleted']) && $filter['deleted'] !== "") {
            $this->db->where('service_type.is_deleted', $filter['deleted']);
        }
        if (isset($filter['status']) && $filter['status'] !== "") {
            $this->db->where('service_type.status', $filter['status']);
        }
        if ($order <> null) {
            $this->db->order_by($order, $dir);
        } else {
    ...
    function get_servicetype_details($id) {
        $id = (int) $id;
        $this->db->select('service_type.*, CONCAT(service_type_created.first_name, " ", service_type_created.last_name) as created_by_name, CONCAT(service_type_updated.first_name, " ", service_type_updated.last_name) as updated_by_name');
        $this->db->from('service_type');
        $this->db->join('users AS service_type_created', 'service_type.created_by = service_type_created.id', 'left');
        $this->db->join('users AS service_type_updated', 'service_type.updated_by = service_type_updated.id', 'left');
        $this->db->where('service_type.id', $id);
        $this->db->where('service_type.is_deleted', 0);
        $query = $this->db->get();
        //echo $this->db->last_query()."<br>";die;
        return $query->row();
    }
