...
class Notify_model extends CI_Model {
	...
	public function get_logs_list($id=null, $filter_logs=null, $count = false){
        $this->db->select('order_schedules.*, orders.order_no, order_schedule_status_log.*,CONCAT(users.first_name, " ", users.last_name) as name');
        $this->db->from('orders');
        ...
        $this->db->where('order_schedule_status_log.log_pdf_admin <>', NULL);
        ...
        if(isset($id)){
            $this->db->where('order_schedule_status_log.order_schedule_id', $id);
        }
        if ($count) {
            $query = $this->db->get();
            return count($query->result());
