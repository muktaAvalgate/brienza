...
class Notify_model extends CI_Model {
	...
	public function get_notifications_list($filter = array(), $order = null, $dir = null, $count = false) {
		...
		$this->db->select('notifications.id, notifications.subject, notifications.description, notifications.type, notification_users.status, notifications.created_on, , CONCAT(users_created.first_name, " ", users_created.last_name) as created_by_name');
		$this->db->from('notifications');
		$this->db->join('users AS users_created', 'notifications.created_by = users_created.id');
		...
		if ( (isset($filter['limit']) && $filter['limit'] > 0) && (isset($filter['offset']) ) ) {
   		...
   		if ($order <> null) {
    		$this->db->order_by($order, $dir);
   			...
   			$this->db->order_by('notification_users.status DESC, notifications.created_on DESC');
    	}
