...
*/
    ...
    **/
    ...
    function check_schedule_30min_diff($presenter_id = '', $start_date='', $end_date='') {
    ...
    **/
    ...
    public function getAdminBilling($prsenter_id,$billing_due_date,$purchase_order_no,$limit,$offset,$session){
        $this->db->select('billing.*,orders.order_no,orders.work_plan_number,orders.hours,orders.hourly_rate,orders.booking_date,user_meta.meta_value as school_name,CONCAT(presenter.first_name," ",presenter.last_name) as presenter_name');
    ...
    public function get_schedule_ids_pdf($id,$old_status,$new_status){
        ...
        $sql ="SELECT attachment FROM `order_schedule_status_log` WHERE `order_schedule_id`=".$id." AND (`old_status`='".$old_status."' AND `new_status`='".$new_status."')";
	...
	public function get_schedule_logdetails($id){
        ...
        $sql ="SELECT school_pdf FROM `order_schedule_status_log` WHERE `order_schedule_id`=".$id." AND (`old_status`='Log sent - awaiting principal signature' AND `new_status`='Create invoice')";
	...
	public function get_presenter_orders_details($order_id,$presenter_id){
        $this->db->select('orders.order_no, orders.school_id, user_meta.meta_value as school_name, titles.name as title_name,order_assigned_presenters.assigned_hours');
        $this->db->from('orders');
        $this->db->join('titles', 'orders.title_id = titles.id');
        $this->db->join('users AS schools', 'orders.school_id = schools.id');
        $this->db->join('user_meta', "user_meta.user_id = schools.id AND user_meta.meta_key = 'school_name'", "left outer");
    ...
    function get_reupload_status($orderId, $presenterId, $sessionTo, $sessionFrorm){
    ...
    public function get_schedule_logContent_txt($id){
        $sql ="SELECT content_txt FROM `order_schedule_status_log` WHERE `order_schedule_id`=".$id." AND (`old_status`='Create log' AND `new_status`='Log sent - awaiting principal signature')";
        $query = $this->db->query($sql);
        //echo $this->db->last_query()."<br>";      
    ...
    public function logs_principal_name($id){
        $sql ="SELECT principal_name FROM `order_schedule_status_log` WHERE `order_schedule_id`=".$id." AND (`old_status`='Create log' AND `new_status`='Log sent - awaiting principal signature')";
        $query = $this->db->query($sql);
        // echo $this->db->last_query()."<br>";  die;    
    ...
    public function custom_tmp($filterLogWritingRm, $count=null){
    ...
    public function get_topic_from_custom_template(){
