            ...
            if ($this->form_validation->run())
            {
                ...
                if($success){
                    ...
                    // start new implementation of deletion of school_titles backup
                        ...
                        $titlesRes = $this->App_model->update('school_titles', 'school_id', $school_id, $data_delete);

                        // 
                        if($titlesRes){
                            // $existing_title = $this->App_model->get_titles_id_for_school_filter($school_id);
							foreach ($titles as $key => $title_id) {
                                $existing_title = $this->App_model->get_existing_title_for_school($school_id,$title_id);
                                if($existing_title != FALSE){
                                    ...
                                    $titleData = array ('school_id' => $school_id, 'title_id' => $title_id);
                    ...
                    $graddelRes = $this->App_model->update('teachers', 'school_id', $school_id, $data_delete);
                    // Add Grades & Teachers
                    foreach ($grades as $title_id => $grade_inner) {
                        foreach ($grade_inner as $index => $grade_id) {
                            if(in_array($title_id ,$titles) ){
                                if ($grade_id <> "") {
                                    ...
                                    $exist_teacher_id = $this->App_model->is_teacher_exist($school_id, $title_id, $grade_id, $teacher_name);
