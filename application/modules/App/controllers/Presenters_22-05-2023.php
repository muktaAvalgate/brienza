	...
	**/
	...
	public function update_scheduling() 
	{
        ...
        if($order_schedules->start_date != $start_date || $order_schedules->end_date != $end_date){
            ...
            $time_mail=$startDate." @ ".$startTime." - ".$endTime." [Previous value: ".$previousStartDate." @ ".$priviousStartTime." - ".$previousEndTime."]";
        ...
        if($order_schedules->grade_id != $this->input->post('grades') || $order_schedules->teacher != $teacherName){
            ...
            $grade_teacher_mail = "Grade & Teacher - ".$new_grade->name." ".$teacherName." [Previous value: ".$order_schedules->grades_name." ".$order_schedules->teacher."]";
        }
        $new_type=$this->App_model->get_worktype_details($this->input->post('types'));
        if($order_schedules->type_id != $this->input->post('types')){
            $type_mail = "Type - ".$new_type->name." [Previous value: ".$order_schedules->type_name."]";
        }
        if ($order_schedules->total_hours != number_format(htmlspecialchars($this->input->post('total_hours'), ENT_QUOTES, 'utf-8'),1)) {
            $total_hours_mail = "Total hours - " . number_format(htmlspecialchars($this->input->post('total_hours'), ENT_QUOTES, 'utf-8'),1) . " [Previous value: " . $order_schedules->total_hours . "]";
        }
		...
		if ($schedule_id = $this->App_model->update('order_schedules', 'id', $schedule_id, $data)) {
			...
			if($order_schedules->start_date != $start_date || $order_schedules->end_date != $end_date || $order_schedules->grade_id != $this->input->post('grades') || $order_schedules->teacher != $teacherName || $order_schedules->type_id != $this->input->post('types') || $order_schedules->total_hours != number_format(htmlspecialchars($this->input->post('total_hours'), ENT_QUOTES, 'utf-8'),1)){
				$subject = "BAASP UPDATE - " . $presenter_name->first_name . " " . $presenter_name->last_name . " - Order: " . $order_number->order_no." - Updated schedule";
				// echo $subject;die;

				$msg = "<p>Hi,</p><br><p>This is to notify that a schedule has been updated as follows:</p><p>".$time_mail."</p><p>".$grade_teacher_mail."</p><p>".$type_mail."</p><p>".$total_hours_mail."</p>";
				//clone mail id
				// $emails = array('brienzaportalstaging@gmail.com','fraidy@thekgroupny.com');
				$emails = array('gs.avalgate@gmail.com','rr.avalgate@gmail.com');
