...
class Schools extends Application_Controller {
    ...
    // public function index_old_27_04_2023() {
    ...
    //     $search_school =$data['search'];
    //     $sortArray_by_school = array();
    //     foreach($data['search'] as $value){
    //       $sortArray_by_school[] = $value->meta['school_name'];
    //     } 
    //     $sortArray_by_school_casesensetive  = array_map('strtolower', $sortArray_by_school);
    //     array_multisort($sortArray_by_school_casesensetive, SORT_ASC, $search_school);
    //     $data['search_school']=$search_school;

    //     // Sort the values in ascending order (Email)
    ...
    public function index() {
		...
		if (strpos($currentURL,'search_box_schl') !== false) {
			// $s_arr = explode('search_box_schl/',$currentURL);
			// $search_box_schl =$s_arr[1];

            $p_arr = explode('/page',$currentURL);
			$s_arr = explode('search_box_schl/',$p_arr[0]);
            $search_box_decoded =$s_arr[1];

			$search_box_schl = base64_decode($search_box_decoded);
			// echo $search_box_schl; 
		} 
        ...
        $filter = array();
		if($search_box_schl != ''){
			$default_uri = array('status', 'page' ,'search_box_schl');
		}else{
			$default_uri = array('status', 'page');
		}
		...
		if($search_box_schl != ''){
			// $filter['search_box_schl'] = $uri['search_box_schl'];
			$pegination_uri['search_box_schl'] = base64_encode($search_box_schl);
			$filter['search_box_schl'] = $search_box_schl;
		}
	    ...
	    // Get the total rows without limit
	    $total_rows = $this->Admin_model->get_users_list_new($filter, 'id', 'asc', true);

        if($search_box_schl != ''){
			
			$pegination_uri = \array_diff($pegination_uri, ["~"]);
			$config = $this->init_pagination('app/schools/index/'.$this->uri->assoc_to_uri($pegination_uri).'/page/', 7, $total_rows);
		}else{
            $config = $this->init_pagination('app/schools/index/'.$this->uri->assoc_to_uri(array('status' => $status)).'/page/', 7, $total_rows);
        }
	    ...
	    // Get the Users List
	    $data['list'] = $this->Admin_model->get_users_list_new($filter, 'id', 'asc');
     		...
     		if ($this->form_validation->run())
     		{
                ...
                if($success){
                    ...
                    if ($this->Admin_model->update($this->tablename, 'id', $id, $data)) {
                        ...
                        $titlesRes = $this->App_model->update('school_titles', 'school_id', $id, $data_delete);

                        // 
                        if($titlesRes){
                            // $existing_title = $this->App_model->get_titles_id_for_school_filter($id);
							foreach ($titles as $key => $title_id) {
                                $existing_title = $this->App_model->get_existing_title_for_school($id,$title_id);
                                if($existing_title != FALSE){
                                    ...
                                    $this->App_model->update('school_titles', 'id', $existing_title->id, $data_restore);
                                }else{
                                    $titleData = array ('school_id' => $id, 'title_id' => $title_id);
								    $this->App_model->insert('school_titles', $titleData);
                                }
                        ...
                        // end new implementation of deletion of school_titles backup
                        ...
                        $graddelRes = $this->App_model->update('teachers', 'school_id', $id, $data_delete);
                        if($graddelRes){
                            foreach ($grades as $title_id => $grade_inner) {
                                foreach ($grade_inner as $index => $grade_id) {
                                    if(in_array($title_id ,$titles) ){
                                        if ($grade_id <> "") {
                                            ...
                                            $exist_teacher_id = $this->App_model->is_teacher_exist($id, $title_id, $grade_id, $teacher_name);
                                            if($exist_teacher_id != FALSE){
                                             $data_restore = array(
                                                    'is_deleted' => 0,
                                                    'updated_by' => $this->session->userdata('id'),
                                                    'updated_on' => date('Y-m-d H:i:s')
                                                    );
                                            $this->App_model->update('teachers', 'id', $exist_teacher_id->id, $data_restore);
                                            }else{
                                                $teacher = array ('school_id' => $id, 'title_id' => $title_id, 'grade_id' => $grade_id, 'name' => $teacher_name, 'created_by' => $this->session->userdata('id'),'created_on' => date('Y-m-d H:i:s'));
                                                $this->App_model->insert('teachers', $teacher);
                                            }  
    ...
    public function grade_teachers_validation(){
        $titles = $this->input->post('titles');
        $grades = $this->input->post('grades');
        $teachers = $this->input->post('teachers');
        $title_key = array();
    ...
    public function check_title_is_assigned(){
        $school_id = $this->input->post('school_id');
        $title_id = $this->input->post('title_id');
        $count = 0;
        $school_titles = $this->App_model->get_school_titles($school_id);
        foreach($school_titles as $k => $v){
            if($k == $title_id){
                $count++;
            }
        }
        if($count > 0){
            $checkIsGradeTeachers = $this->App_model->get_title_details($title_id);
            // print_r($checkIsGradeTeachers);
            if($checkIsGradeTeachers->grade_teachers == 1){
                echo 2;
            }else{
                echo 3;
            }
