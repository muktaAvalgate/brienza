...
class Notifications extends Application_Controller {
	...
	public function index() {
        ...
        if ($folder == "sent") {
            ...
            $filter_logs = array('delete' => '0', 'category' => 'logs', 'created_by' => $this->session->userdata('id'));
		...
		if ($folder == "logs"){
            $total_rows = $this->Notify_model->get_logs_list($id=null, $filter_logs=null,true);
            ...
            $filter_logs['limit'] = $config['per_page'];
            $filter_logs['offset'] = $limit_end;

            // Get the Permissions List
            $data['unread_logs'] = $this->Notify_model->get_logs_list(null,$filter_logs,false);
            
            
        }
        // ......................................
        $data['log_details'] = $this->Notify_model->get_log_details();
        $filter_announcement = array(
            'delete' => 0,
            'category' => 'announcement',
            'status' => 'unread',
	...
	public function log_download($id = NULL, $status = NULL) {
        $record = $this->Notify_model->get_logs_list($id);
        $log_status=$this->Notify_model->update_log_status($record[0]->id);
        $file = $record[0]->content_for_print;
        
        if (!empty($record[0]) && $record[0]->attachment!=NULL) {
            $file = DIR_TEACHER_FILES.$record[0]->attachment;
            if (file_exists($file)) {
                // get file content
                $data = file_get_contents ( $file );
                //force download
                $this->load->helper('download');
                force_download ( $record[0]->attachment, $data );
            }
