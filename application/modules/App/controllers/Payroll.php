...
class Payroll extends Application_Controller 
{
	...
	*/
	public function show_payable_schedules($id)
	{
		...
		//2021-08-03------start
		$crr_row = $this->App_model->get_payementDetails_bypid($id);
		$get_schedule_details_nolate = $this->App_model->get_schedules_byPaymentdetails_nolate($crr_row->session_from, $crr_row->session_to);
		//$get_schedule_details_late = $this->App_model->get_schedules_byPaymentdetails_late($crr_row->session_from, $crr_row->session_to);
		$order_schedule_ids2 = array();
		if(!empty($get_schedule_details_nolate)){
			foreach($get_schedule_details_nolate as $row){
				$order_schedules_ids[] = $row->id;
			}
			$schedule_detailsFrm_odrdetls = $this->App_model->get_schedule_detailsFrm_odrdetls($order_schedules_ids, $crr_row->billing_date);
			// echo "<pre>";print_r($schedule_detailsFrm_odrdetls);die;
			
			if(!empty($schedule_detailsFrm_odrdetls)){
				foreach($schedule_detailsFrm_odrdetls as $row2){
					$order_schedule_ids2[] = $row2->order_schedule_id;
				}
			}
		}
		//2021-08-03------end

		//2021-07-30---------start 1st implement
		//$crr_row = $this->App_model->get_payementDetails_bypid($id);
		$pre_row = $this->App_model->get_pre_payementDetails_bypid($id);
		...
		if(!empty($get_schedule_details)){
			...
			// echo "<pre>";print_r($order_schedule_ids);die;
			$final_schedule_ids = array_merge($order_schedule_ids2,$order_schedule_ids);
			//echo "<pre>";print_r($final_schedule_ids);die;
			$processed = $this->Payroll_model->get_payable_schedules_order_schedules_new($final_schedule_ids, $crr_row->session_to);
			// echo "<pre>";print_r($processed);die;
			foreach ($processed as $key => $val) {          
				$logData = $this->Payroll_model->get_schedule_logs($val->id);
				$processed[$key]->order_log = $logData;
				//added for presenter
                $presenter_data = $this->Payroll_model->get_presenter_data($val->created_by);
                $processed[$key]->presenter_data = $presenter_data;
			}
			$data['order'] = $processed;
		}else{
			if(!empty($order_schedule_ids2)){
				$processed = $this->Payroll_model->get_payable_schedules_order_schedules_new($order_schedule_ids2, $crr_row->session_to);
				// echo "<pre>";print_r($processed);die;
				foreach ($processed as $key => $val) {          
					$logData = $this->Payroll_model->get_schedule_logs($val->id);
					$processed[$key]->order_log = $logData;
	...
	public function download_csv($payment_schedule_id=0){
        ...
        if(!empty($pScheduleData)){
			...
			foreach ($processed as $key => $row) {
				//added for presenter
                $presenter_data = $this->Payroll_model->get_presenter_data($row->created_by);
                $processed[$key]->presenter_data = $presenter_data;
                //------end-----------
                $session = date_display($row->start_date, "m/d/Y").' @ '.time_display($row->start_date, true).'-'.time_display($row->end_date, true).' with '.$row->teacher;
                $lineData = array($session, $row->order_no, $row->presenter_data->first_name, $row->presenter_data->email, $row->presenter_data->meta_value, $row->status);
                fputcsv($f, $lineData, $delimiter);
            }
