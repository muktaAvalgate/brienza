...
class Presenters extends Application_Controller {
	...
	// public function index_03_05_2023() {
    	...
    	if ($this->input->server('REQUEST_METHOD') === 'POST')
    	{
    		...
    		if ($this->form_validation->run())
    		{
    			...
    			if ($user_id = $this->Admin_model->insert($this->tablename, $data)) {
                    ...
                    //$this->mail_template->notification_email(null, 'kconte@brienzas.com', $msg, 'Presenter');
	...
	**/
	...
	public function update_scheduling() 
	{
		...
		if($status == 'approved'){
			...
			if($updatedate = $this->App_model->update('order_schedules', 'id', $order_schedule_id, $new_data)){
				...
				if($order_schedules->start_date != $new_data['start_date'] || $order_schedules->end_date != $new_data['end_date']){
					...
					// $total_hours_mail = "Total hours - " . $order_schedules->total_hours . " <i>[Previous value: " . $order_schedules->total_hours . "]</i>";
					// $msg = "<p>Hi,</p><p>This is to notify that a schedule has been updated.</p>";
					// mail id
					$msg = "<p>Hi,</p><p>This is to notify that a schedule has been updated as follows:</p><p>".$time_mail."</p>";
					...
					// $emails = array('gs.avalgate@gmail.com');
					// $emails = array('DMaddaloni@brienzas.com');
					// $emails = array('fraidy@thekgroupny.com');
		...
		$public_school_title=$this->App_model->public_school_title($order_id); // 23-05-2023
	...
	/* public function billing() {
		...
		// if(!$order_id) {
		// 	$data['main_content'] = 'presenters/demo-view';
			//$this->load->view(TEMPLATE_PATH, $data);
		// }
	...
	// public function billing_bfr_public($para = NULL){
	// 	$data['billing'] = false;
    //     $presenter_id = $this->session->userdata('id');
					...
					foreach($check_is_unique_invoice_id as $key => $inv_id){
						...
						if($session_to < $current_date && $order_schedules_without_public == 1){
							...
							if($scheduleHours){
								...
								// 24-05-2023
								if($inv_id->invoice_id == 0){
									$invoicePath_new = $this->App_model->get_invoice_new($val['order_id'], $val['bill_period_start_date'], $sessionEndDate,$presenter_id);
								}else{
	...
	public function logs_13_03_2023() {
