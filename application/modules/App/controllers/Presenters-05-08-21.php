	...
	**/
	...
	public function billing() {
        ...
        foreach($orderArray as $key => $value){
            ...
            // echo '<pre>'; print_r($ordDtls); die();
            $invoice_createdDate = $this->App_model->get_invoice_created_date($key);
                ...
                if($session_to < $current_date){
                    if($scheduleHours){
                        ...
                        if($othersDetails){
                            ...
                            if($created_date == 0){
                                $current_date = date("Y-m-d");
                                if($current_date > $completeDateOriginal){
                                    $lateFlag = 1;
                                }
	...
	public function logs() {
		...
		$default_uri = array('page, order_no, school, presenter');
    	$uri = $this->uri->uri_to_assoc(4, $default_uri);
	    ...
	    $config = $this->init_pagination('app/presenters/logs/'.$this->uri->assoc_to_uri($pegination_uri).'/page/', 11, $total_rows);
