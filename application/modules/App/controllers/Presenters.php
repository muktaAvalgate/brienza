	...
	**/
	public function scheduling() 
	{
		...
		if ($order_id) 
		{
			...
			// echo '<pre>'; print_r($s_date);
			// echo '<pre>'; print_r($e_date);
			// echo '<pre>'; print_r($data['startDate']);
			// echo '<pre>'; print_r($data['endDate]); 
	...
	public function update_scheduling() 
	{
		...
		if($status == 'approved'){
			$date = $this->input->post('updated_date'); 
			$dateTime = new DateTime($date);
			  echo '<pre>';print_r($dateTime);
			$order_schedule_id = $this->input->post('order_schedule_id');
			//   echo '<pre>';print_r($order_schedule_id);die;
			$order_schedules = $this->App_model->get_order_schedule_formail($order_schedule_id);
			 echo "<pre>"; print_r($order_schedules);echo '</pre>';
			 $order_number = $this->App_model->get_order_number_formail($order_schedules->order_id);
			$data = array(
				'start_date' => $order_schedules->start_date,
				'end_date' => $order_schedules->end_date,
			);
			// echo "<pre>"; print_r($data);die;
			$onlyDateFromOldDate=date('m/d/Y', strtotime($data['start_date']));
			echo "<pre>"; print_r($onlyDateFromOldDate);
			$startTimeFromOldDate=date('h:i A', strtotime($data['start_date']));
			$endTimeFromOldDate=date('h:i A', strtotime($data['end_date']));
			$onlyDateFromNewDate=$dateTime->format('m/d/Y');
			$newTime = date('H:i:s', strtotime($data['start_date']));
			$newtime = date('H:i:s', strtotime($data['end_date']));
			$new_data['start_date'] = $dateTime->format('Y-m-d') . ' ' . $newTime;
            $new_data['end_date'] = $dateTime->format('Y-m-d') . ' ' . $newtime;
			echo "<pre>"; print_r($order_schedules->total_hours); 

			if($updatedate = $this->App_model->update('order_schedules', 'id', $order_schedule_id, $new_data)){
					//Send notification to admin update schedule
				if($order_schedules->start_date != $new_data['start_date'] || $order_schedules->end_date != $new_data['end_date']){

					$time_mail=$onlyDateFromNewDate." @ ".$startTimeFromOldDate." - ".$endTimeFromOldDate." <i>[Previous value: ".$onlyDateFromOldDate." @ ".$startTimeFromOldDate." - ".$endTimeFromOldDate."]</i>";
					echo $time_mail;die();
					// $type_mail = "Type - ".$new_type->name." <i>[Previous value: ".$order_schedules->type_name."]</i>";
					
					$topic_mail = "Topic - ".$order_schedules->topic_name." <i>[Previous value: ".$order_schedules->topic_name."]</i>";
					
					$total_hours_mail = "Total hours - " . $order_schedules->total_hours . " <i>[Previous value: " . $data['total_hours'] . "]</i>";
					// $msg = "<p>Hi,</p><p>This is to notify that a schedule has been updated.</p>";
					// mail id
					$msg = "<p>Hi,</p><p>This is to notify that a schedule has been updated as follows:</p><p>".$time_mail."</p><p>"."<p>".$topic_mail."</p><p>".$total_hours_mail."</p>";
					...
					$emails = array('mg.avalgate@gmail.com');
		...
		foreach($finalArray as $key => $val){
			...
			if($check_is_unique_invoice_id != false){
					...
					foreach($check_is_unique_invoice_id as $key => $inv_id){
						...
						$check_past_schedules_for_public = $this->App_model->get_order_schedule_with_range_with_public($val['order_id'], $presenter_id, $val['bill_period_start_date'],$sessionEndDate,$inv_id->invoice_id,$schedules_id,'2');
						...
						if($session_to < $current_date && $order_schedules_without_public == 1){
							...
							// total hours scheduled by invoice id
							$scheduleHours = $this->App_model->get_scheduled_hours_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id,$schedules_id,'0');
							
							// total scheduled hours by invoice id whose status is confirm. i.e after scheduling status should be confirm hours or after that.
							$scheduleHoursConfirm = $this->App_model->get_scheduled_hours_confirm_by_inv_id($val['order_id'], $presenter_id,$inv_id->invoice_id,$schedules_id,'0');

							//19-04-2023
							$statusCreateInvoice = $this->App_model->get_status_createInvoice_by_inv_id_range($val['order_id'], $presenter_id, $inv_id->invoice_id,$val['bill_period_start_date'],$sessionEndDate, 'unchecked',$schedules_id);

							$no_of_rows = $this->App_model->no_of_rows_by_inv_id_range($val['order_id'], $presenter_id, $inv_id->invoice_id,$val['bill_period_start_date'],$sessionEndDate, 'unchecked',$schedules_id);
	...
	public function viewTitleDetails_ajax(){
		...
		if($order_no != 'false'){
			$title_data['topics'] = $this->App_model->get_title_topics_des_order_wise($title_id, $order_no); //returned false from model when list is empty
		
		}else{
			$title_data['topics'] = $this->App_model->get_title_topics_des($title_id); //returned false from model when list is empty
		}
	...
	public function add_log_template(){
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST')
		{
			...
			if ($this->form_validation->run())
			{
				...
				if ($insert_id = $this->App_model->insert('', $data)){
