...
class Servicetype extends Application_Controller {
    ...
    private $url = '/app/servicetype';
    private $permissionValues = array(
 		'index' => 'App.Servicetype.View',
 		'add' => 'App.Servicetype.Add',
 		'edit' => 'App.Servicetype.Edit',
        'delete' => 'App.Servicetype.Delete',
        'update_status' => 'App.Servicetype.UpdateStatus',
    );
    ...
    public function index() {
        ...
        $data['list'] = $this->App_model->get_servicetype_list(array('deleted' => 0));
        $data['page'] = 'service';
    	$data['page_title'] = SITE_NAME.' :: service Management';

    	$data['main_content'] = 'servicetype/list';
    	...
    	if ($this->input->server('REQUEST_METHOD') === 'POST')
    	{
    		...
    		if ($this->form_validation->run())
    		{
    			...
    			if ($this->App_model->insert($this->tablename, $data_to_store)) {
    				$this->session->set_flashdata('message_type', 'success');
    				$this->session->set_flashdata('message', '<strong>Well done!</strong> Service type has been added successfully.');
    			} else {
    				$this->session->set_flashdata('message_type', 'danger');
    				$this->session->set_flashdata('message', '<strong>Oh snap!</strong> Service type already exists.');
    			}
    			redirect('/app/Servicetype/');
    		} //validation run
    	}

        $data['page'] = 'servicetype';
    	$data['page_title'] = SITE_NAME.' Service type Management &raquo; Add Service type';

        $data['main_content'] = 'servicetype/add';
    	$this->load->view(TEMPLATE_PATH, $data);
    }
	...
	public function edit($id = 0) {
		...
		parent::checkMethodPermission($this->permissionValues[$this->router->method]);
     	...
     	if ($this->input->server('REQUEST_METHOD') === 'POST')
     	{
    		...
    		$this->form_validation->set_rules('name', 'Grade Name', 'trim|required');
     		...
     		if ($this->form_validation->run())
     		{
     			...
     			if ($this->App_model->update($this->tablename, 'id', $id, $data_to_store)) {
     				$this->session->set_flashdata('message_type', 'success');
     				$this->session->set_flashdata('message', '<strong>Well done!</strong> Service type successfully updated.');
     			} else{
     				$this->session->set_flashdata('message_type', 'danger');
     	...
     	$data['grade'] = $this->App_model->get_servicetype_details($id);
        ...
        $data['main_content'] = 'servicetype/edit';
    	$this->load->view(TEMPLATE_PATH, $data);
    }
    ...
    public function delete($id = null) {
    	...
    	$data['info'] = $this->App_model->get_servicetype_details($id);
		...
		$data_to_store = array(
      	...
      	if ($this->App_model->update($this->tablename, 'id', $id, $data_to_store)) {
            $this->session->set_flashdata('message_type', 'success');
    		$this->session->set_flashdata('message', '<strong>Well done!</strong> Service type successfully deleted.');
        } else {
            $this->session->set_flashdata('message_type', 'danger');
            $this->session->set_flashdata('message', '<strong>Oh snap!</strong> Change something and try again.');
        }
    ...
    public function update_status() {
    	...
    	if ($this->input->server('REQUEST_METHOD') === 'POST')
    	{
    		...
    		if ($this->form_validation->run())
    		{
    			...
    			$this->session->set_flashdata('message_type', 'success');
    			$this->session->set_flashdata('message', '<strong>Well done!</strong> '.$count.' Service type(s) successfully '.$msg);
