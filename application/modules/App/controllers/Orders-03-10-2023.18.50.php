...
class Orders extends Application_Controller {
    ...
    **/
    ...
    $existingFile=$config['upload_path'] = DIR_TEACHER_FILES.$config['file_name'].'.pdf';
                        //print "<pre>"; print_r($_FILES['attach[]']); print "</pre>"; die;
                        $this->upload->initialize($config);
    
                        if ($this->upload->do_upload('attach[]')) {

                            if(file_exists($existingFile) && $attachment['type']=='application/pdf'){
                                
                                $compressed_file_path = DIR_TEACHER_FILES . $_FILES['file']['name'] . '-compressed.pdf';
                                ...
                                // $file = $myTask->addFile('E:/Problem_PDF/Problem_PDF/log_attachment-142915020120230608185656.pdf');
                                $file = $myTask->addFile( $existingFile);
                                // echo '<pre>';print($file);die;
                                $myTask->execute();

                                $myTask->download($compressed_file_path);
                                unlink($existingFile);
                            }
