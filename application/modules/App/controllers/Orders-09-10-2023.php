  ...
  /*  public function reUpload_document_for_presenter() {
    ...
    public function billing_merge() {
        ...
        foreach ($data['pdfmerge_data'] as $k) {
            ...
            if (!isset($attachmentCounts[$invoice_id])) {
                $attachmentCounts[$invoice_id] = 0; 
            }
            if ($pdf_merge_status == 0) {
                $attachmentCounts[$invoice_id]++;
            }
        }
        // $unique_invoice_ids = array_unique(array_column($data['pdfmerge_data'], $attachment_name));
        // foreach ($unique_invoice_ids as $invoice_id) {
        //     if (!isset($attachmentCounts[$invoice_id])) {
        //         $attachmentCounts[$invoice_id] = 0;
        //     }
        // }
