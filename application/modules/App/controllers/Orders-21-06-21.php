...
class Orders extends Application_Controller {
    ...
    **/
	...
	public function billing() {
		...
		$scheduled_ids = $this->App_model->get_schedule_ids($order_id);
   ...
   private function format_date($date) {
	...
	public function preview_invoice() {
		...
		// print "<pre>"; print_r($data); print "</pre>";exit;
		$data['isBilling'] = $this->App_model->get_billing_order_count($order_id);
		$data['page'] = 'order';
    	$data['page_title'] = SITE_NAME.' :: Order Management &raquo; Billing';
	...
	public function save_billing(){
		...
		$order = $this->App_model->get_order_details($data['order_id']);
							...
							<table class="table table-responsive table-hover sub-order" width="100%">
						        ...
						        if(count($schedules)>0){
						        	...
						        	$invoice.='<tr style="background-color: #e9e9e9">
							        		...
							        		<td style="border:2px solid #763199;background-color: #51c03de3;"><span style="background-color: #fff;padding: 10px 20px;">$'.$row->hourly_rate.'</span></td>
							        		<td style="border:2px solid #763199;background-color: #51c03de3;"><span style="background-color: #fff;padding: 10px 20px;">$'.$row->hourly_rate*$row->total_hours.'</span></td>
    ...
    function billingProcess(){
    	...
    	foreach ($scheduled_ids	 as $row) {
    		...
    		if(file_exists(DIR_TEACHER_FILES.$pdf1)){
