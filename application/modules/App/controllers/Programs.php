...
class Programs extends Application_Controller {
	 ...
	 */
 	 ...
 	 * @var unknown_type
 	 */
    private $tablename = 'programs';
    private $url = '/app/programs';
    private $permissionValues = array(
 		'index' => 'App.Programs.View',
 		'add' => 'App.Programs.Add',
 		'edit' => 'App.Programs.Edit',
        'delete' => 'App.Programs.Delete',
        'update_status' => 'App.Programs.UpdateStatus'
    );
    ...
    public function index() {
        ...
        $data['list'] = $this->App_model->get_program_list($search);
        // echo '<pre>';print_r($data['list']);echo '</pre>';die;
        $data['page'] = 'programs';
    	$data['page_title'] = SITE_NAME.' :: Programs Management';

    	$data['main_content'] = 'programs/program_list';
    ...
    public function add() {
    	...
    	if ($this->input->server('REQUEST_METHOD') === 'POST')
    	{
    		...
    		{
    			$data_to_store = array(
    				'name' => htmlspecialchars($this->input->post('program_name'), ENT_QUOTES, 'utf-8'),
    			...
    			if ($this->App_model->insert($this->tablename, $data_to_store)) {
    				...
    				$this->session->set_flashdata('message', '<strong>Well done!</strong> Program has been added successfully.');
    			} else {
    				$this->session->set_flashdata('message_type', 'danger');
    				$this->session->set_flashdata('message', '<strong>Oh snap!</strong> Program already exists.');
    			}
        ...
        $data['page'] = 'programs';
    	$data['page_title'] = SITE_NAME.' Program Management &raquo; Add Program';

        $data['main_content'] = 'programs/add';
    	$this->load->view(TEMPLATE_PATH, $data);
    }
	...
	public function edit($id = 0) {
    		...
    		$this->form_validation->set_rules('name', 'Program Name', 'trim|required');
     		...
     		if ($this->form_validation->run())
     		{
     			...
     			if ($this->App_model->update($this->tablename, 'id', $id, $data_to_store)) {
     				$this->session->set_flashdata('message_type', 'success');
     				$this->session->set_flashdata('message', '<strong>Well done!</strong> Program successfully updated.');
     			} else{
     				$this->session->set_flashdata('message_type', 'danger');
        ...
        //print "<pre>"; print_r($data['list']); print "</pre>"; die;
     	$data['page'] = 'programs';
    	$data['page_title'] = SITE_NAME.' Program Management &raquo; Edit Program';

        $data['main_content'] = 'programs/edit';
    	$this->load->view(TEMPLATE_PATH, $data);
    }
    ...
    public function delete($id = null) {
    	...
    	$data['info'] = $this->App_model->get_program_details($id);
		...
		$data_to_store = array(
      	...
      	if ($this->App_model->update($this->tablename, 'id', $id, $data_to_store)) {
            $this->session->set_flashdata('message_type', 'success');
    		$this->session->set_flashdata('message', '<strong>Well done!</strong> Program successfully deleted.');
        } else {
            $this->session->set_flashdata('message_type', 'danger');
            $this->session->set_flashdata('message', '<strong>Oh snap!</strong> Change something and try again.');
        }
    		...
    		if ($this->form_validation->run())
    		{
    			...
    			foreach ($items as $id=>$value) {
					...
					if ($operation == 'delete') {
                        ...
                        $data['info'] = $this->App_model->get_program_category_details($id);
    			...
    			$this->session->set_flashdata('message_type', 'success');
    			$this->session->set_flashdata('message', '<strong>Well done!</strong> '.$count.' program(s) successfully '.$msg);
