...
class Presenters extends Application_Controller {
	...
	//     $filter = array();
	// 	$default_uri = array('status', 'page' ,'presenter' , 'company', 'email');
	...
	// 	$search_email =$data['search'];
    //     $sortArray_by_email = array();
    //     foreach($data['search'] as $value){
    //       $sortArray_by_email[] = $value->email;
    //     }
    //     $sortArray_by_email_casesensetive  = array_map('strtolower', $sortArray_by_email);
    //     // Sort the values in ascending order
    //     array_multisort($sortArray_by_email_casesensetive, SORT_ASC, $search_email);
	...
	public function index() {
		...
		if (strpos($currentURL,'search_box') !== false) {
			// $s_arr = explode('/',$currentURL);
			// $search_box =$s_arr[7];

			// $s_arr = explode('search_box/',$currentURL);

			$p_arr = explode('/page',$currentURL);
			$s_arr = explode('search_box/',$p_arr[0]);

			$search_box_decoded =$s_arr[1];
			$search_box = base64_decode($search_box_decoded);
		} 

	    $filter = array();
		if($search_box != ''){
			$default_uri = array('status', 'page' ,'search_box');
		}else{
		...
		if($search_box != ''){
			$pegination_uri['search_box'] = base64_encode($search_box);
			$filter['search_box'] = $search_box;
		...
		if($search_box != ''){
			...
			$config = $this->init_pagination('app/presenters/index/'.$this->uri->assoc_to_uri($pegination_uri).'/page/', 7, $total_rows);
		}else{
			$pegination_uri = \array_diff($pegination_uri, ["~"]);
			$config = $this->init_pagination('app/presenters/index/'.$this->uri->assoc_to_uri($pegination_uri).'/page/', 5, $total_rows);
		}
	...
	**/
	public function scheduling() 
	{
		...
		if ($order_id) 
		{
			...
			// get title name 11-07-2023
			$data['title_name'] = $this->App_model->get_title_name($data['order']->title_id); 
            // Get the Topics
			$data['topics'] = $this->App_model->get_title_topics($data['order']->title_id); 
			...
			if($order_id != ''){
				...
				$data['title_id'] = $this->App_model->get_title_id($order_id);
				$public_school_title_status = $this->App_model->get_title_status($data['title_id']);
			}
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST')
    	{
			...
			$this->form_validation->set_rules('total_hours', 'Total hours', 'trim|required');
     		...
     		if ($this->form_validation->run())
     		{
				...
				if(($checkDateIfExitsSubmittedInvoice == true) && ($data['order']->public_school_title_status != 'checked')){
					$this->session->set_flashdata('message_type', 'danger');
                ...
                // }
     			...
     			$data = array(
					...
					'total_hours' => htmlspecialchars($this->input->post('total_hours'), ENT_QUOTES, 'utf-8'),
     				'created_by' => $this->session->userdata('id'),
     				'created_on' => date('Y-m-d H:i:s'),
					'is_public_school' => $public_school_title_status,
					'is_old_order' => '1'
    			);
				...
				if ($schedule_id = $this->App_model->insert('order_schedules', $data)) {
                    // ======== Start Code By Sudip on 2023-05-17 ======= //
                    // Send notification on Site admin

                    $msg = "<p>Hi,</p><p>This is to notify that a new schedule has been added.</p>";
                    
                    $subject = "BAASP UPDATE - " . $presenter_name->first_name . " " . $presenter_name->last_name . " - Order: " . $order_number->order_no." - New schedule";
                    //clone mail id
            		 $emails = array('brienzaportalstaging@gmail.com');
					// $emails = array('gs.avalgate@gmail.com','rr.avalgate@gmail.com');
					$this->load->library('mail_template');
                    $this->mail_template->notification_email_for_add_schedule($emails, null, $subject, $msg);
		...
		$this->session->set_userdata('scheduling_link_for_cancel', $actual_link);
	...
	public function update_scheduling() 
	{
		...
		$public_school_title=$this->App_model->public_school_title($order_id); // 21-04-2023
        ...
        // ===== Start code by Ahmed on 2019-08-07 ====== //
        $diff30Min = $this->App_model->check_schedule_30min_diff($this->session->userdata('id'), $start_date, $end_date,$schedule_id);
        //print_r($diff30Min);die;
        if(!empty($diff30Min)){
        ...
        $type_mail=NULL;
        $total_hours_mail=NULL;
		$topic_mail=NULL;
        if($order_schedules->start_date != $start_date || $order_schedules->end_date != $end_date){
            ...
            $previousdateTime = new DateTime($order_schedules->start_date);
            $previousStartDate = $previousdateTime->format('m/d/Y');

            $previousStartDateTime = new DateTime($order_schedules->start_date);
            $priviousStartTime = $previousStartDateTime->format('h:i A');

            $previousEndDateTime = new DateTime($order_schedules->end_date);
            $previousEndTime = $previousEndDateTime->format('h:i A');

            // 02/27/2023 @ 02:00 pm - 07:00 pm [Previous value: 02/26/2023 @ 02:00 pm - 06:00 pm]
            $time_mail=$startDate." @ ".$startTime." - ".$endTime." <i>[Previous value: ".$previousStartDate." @ ".$priviousStartTime." - ".$previousEndTime."]</i>";

            // echo $time_mail;die;
        }
        $new_grade=$this->App_model->get_grade_name($this->input->post('grades'));
        if($order_schedules->grade_id != $this->input->post('grades') || $order_schedules->teacher != $teacherName){
            // Grade & Teacher - 5th Mrs. Testing[Previous value: 6th Mrs. FG]
		...
		$new_topic=$this->App_model->get_topic_name($this->input->post('topics'));
        if($order_schedules->topic_id != $this->input->post('topics')){
            $topic_mail = "Topic - ".$new_topic->topic." <i>[Previous value: ".$order_schedules->topic_name."]</i>";
        }
        if ($order_schedules->total_hours != number_format(htmlspecialchars($this->input->post('total_hours'), ENT_QUOTES, 'utf-8'),1)) {
            $total_hours_mail = "Total hours - " . number_format(htmlspecialchars($this->input->post('total_hours'), ENT_QUOTES, 'utf-8'),1) . " <i>[Previous value: " . $order_schedules->total_hours . "]</i>";
        }
		...
		if ($schedule_id = $this->App_model->update('order_schedules', 'id', $schedule_id, $data)) {
			...
			if($order_schedules->start_date != $start_date || $order_schedules->end_date != $end_date || $order_schedules->grade_id != $this->input->post('grades') || $order_schedules->teacher != $teacherName || $order_schedules->type_id != $this->input->post('types') || $order_schedules->topic_id != $this->input->post('topics') || $order_schedules->total_hours != number_format(htmlspecialchars($this->input->post('total_hours'), ENT_QUOTES, 'utf-8'),1)){
				...
				// $msg = "<p>Hi,</p><p>This is to notify that a schedule has been updated as follows:</p><p>".$time_mail."</p><p>".$grade_teacher_mail."</p><p>".$type_mail."</p><p>".$total_hours_mail."</p>";
				$msg = "<p>Hi,</p><p>This is to notify that a schedule has been updated as follows:</p><p>".$time_mail."</p><p>".$grade_teacher_mail."</p><p>".$type_mail."</p><p>".$topic_mail."</p><p>".$total_hours_mail."</p>";
				//clone mail id
				// $emails = array('brienzaportalstaging@gmail.com','fraidy@thekgroupny.com');
				$emails = array('brienzaportalstaging@gmail.com');
				// $emails = array('gs.avalgate@gmail.com','rr.avalgate@gmail.com');
				// $emails = array('bm.avalgate@gmail.com','skd.avalgate@gmail.com');
	...
	public function show_calendar() {
		...
		if ($order_id) {
			...
			$public_school_title=$this->App_model->public_school_title($order_id);
			$public_school_title_status = $public_school_title->public_school_title_status;
			$data['holidays']['public_school_title'] = $public_school_title_status;
	...
	// public function billing_old_20_04_2023($para = NULL) {
	// 	$data['billing'] = false;
    //     $presenter_id = $this->session->userdata('id');
    ...
    //             if($session_to < $current_date  && $ordDtls->public_school_title_status != 'checked'){
    //                 if($scheduleHours){
    //                     // echo $ordDtls->order_no; echo '+++++++++++++++++++';
	...
	// 					// 31-03-2023
	// 					$invoicePath_new = $this->App_model->get_invoice_new($key, $val, $sessionEndDate,$presenter_id);
	// 					// print_r($invoicePath); die();
	// 					if(isset($invoicePath_new)){
	// 						$doc = $invoicePath_new->invoice_document;
	// 					}else{
	// 						$doc = 0;
	// 					}
	// 					$displayArray['invoice_document'] = $doc;
	...
	// 					}else if($ordDtls->public_school_title_status == 'checked'){
    //                         $displayArray['billing_period'] = $billing_period_s.' to '.$billing_period_e;
    //                         $displayArray['completed_by'] = 'N/A';
	...
	// 	// 07-04-2023
	// 	$finalArrayForPublicSchoolTitle = array();
	// 	foreach($finalArray as $key => $val){
	// 		$schedules_id = array();
	// 		//getting schedules ids in array
	// 		foreach($val['order_schedules'] as $schedules){
	// 			// echo $key; echo $schedules->invoice_id; echo '</br>';
	// 			$schedules_id[]=$schedules->id;
	// 		}
	// 		// checking unique invoice ids without 0 values
	// 		$check_is_unique_invoice_id = $this->App_model->check_is_unique_invoice_id($schedules_id);
	// 		// echo '<pre>'; echo $check_is_unique_invoice_id; die;

	// 		if($check_is_unique_invoice_id != false){
	// 				// getting order_no,school_id,school_name,title_name,assigned_hours by order_id and presenter
	// 				$ordDtls =  $this->App_model->get_presenter_orders_details($val['order_id'],$presenter_id);
					
	// 				$displayArray = array();
	// 				$order_schedules = array();
	// 				// foreach($value as $val){
	// 				foreach($check_is_unique_invoice_id as $key => $inv_id){
	// 					// total hours scheduled by invoice id
	// 					$scheduleHours = $this->App_model->get_scheduled_hours_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id);
						
	// 					// total scheduled hours by invoice id whose status is confirm. i.e after scheduling status should be confirm hours or after that.
	// 					$scheduleHoursConfirm = $this->App_model->get_scheduled_hours_confirm_by_inv_id($val['order_id'], $presenter_id,$inv_id->invoice_id);
						
	// 					// getting status of orders by invoice_id whose status is 'create invoice'.
	// 					// $statusCreateInvoice = $this->App_model->get_status_createInvoice_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id);
						
	// 					// // counting total on of records by invoice_id.
	// 					// $no_of_rows = $this->App_model->no_of_rows_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id);
	// 					// // condition to check if submit invoice button be enable or disable.
	// 					// if($statusCreateInvoice == $no_of_rows){
	// 					// 	$submitInvoiceCounter = 1;
	// 					// }else{
	...
	// 					$invoice_createdDate = $this->App_model->get_invoice_created_date($val['order_id'], $val['bill_period_start_date']);
	...
	// 					$oldrSubInv = $this->App_model->older_submitted_invoice_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id);
	// 					// getting pshedule_id,session_from,session_to,billing_date,payment_date,invoice_no,email_remonder_date,month,year,is_deleted  by checking 'session from' date
	// 					$othersDetails = $this->App_model->get_billing_date($val['bill_period_start_date']);
	// 					// echo '<pre>'; print_r($othersDetails); die();
	// 					//payement schedule is set or not. 
	...
	// 					$dateArray = explode("-", $val['bill_period_start_date']);
	// 					$year = $dateArray[0];
	// 					$month = $dateArray[1];
	// 					// echo '<pre>'; print_r($dateArray); die();
	// 					$days = cal_days_in_month(CAL_GREGORIAN,$month,$year);
	// 					$billing_period_s = date("m/d/y", strtotime($val['bill_period_start_date']));

	// 					$flag = date("d", strtotime($val['bill_period_start_date'])) == '01' ? 1 : 2;

	// 					if($flag == 1){
	// 						$billing_period_e = date("m/d/y", strtotime($val['bill_period_start_date']. ' + 14 days'));
	// 						$daysToAdd = '14';
	// 					}else if($days == 31){
	// 						$billing_period_e = date("m/d/y", strtotime($val['bill_period_start_date']. ' + 15 days'));
	// 						$daysToAdd = '15';
	// 					}else if($days == 29){
	// 						$billing_period_e = date("m/d/y", strtotime($val['bill_period_start_date']. ' + 13 days'));
	// 						$daysToAdd = '13';
	// 					}else if($days == 28){
	// 						$billing_period_e = date("m/d/y", strtotime($val['bill_period_start_date']. ' + 12 days'));
	// 						$daysToAdd = '12';
	// 					}else{
	// 						$billing_period_e = date("m/d/y", strtotime($val['bill_period_start_date']. ' + 14 days'));
	// 						$daysToAdd = '14';
	// 					}
	// 					// calculation for session period //$val->session from date
	// 					$endDate = strtotime("+".$daysToAdd." days", strtotime($val['bill_period_start_date']));
	// 					$sessionEndDate = date("Y-m-d", $endDate);

	// 					// $statusCreateInvoice = $this->App_model->get_status_createInvoice_by_inv_id_range($val['order_id'], $presenter_id, $inv_id->invoice_id,$val['bill_period_start_date'],$sessionEndDate);

	// 					// $no_of_rows = $this->App_model->no_of_rows_by_inv_id_range($val['order_id'], $presenter_id, $inv_id->invoice_id,$val['bill_period_start_date'],$sessionEndDate);
	...
	// 					$order_schedules_without_public = $this->App_model->get_order_schedule_with_range_without_public($val['order_id'], $presenter_id, $val['bill_period_start_date'],$sessionEndDate, "order_schedules.id",$inv_id->invoice_id);

	// 					$order_schedules_with_public = $this->App_model->get_order_schedule_with_range_with_public($val['order_id'], $presenter_id, $val['bill_period_start_date'],$sessionEndDate, "order_schedules.id",$inv_id->invoice_id);

	// 					$check_past_schedules_for_public = $this->App_model->get_order_schedule_by_inv_id_by_time($val['order_id'], $presenter_id, $inv_id->invoice_id, "order_schedules.id",$val['bill_period_start_date'],$sessionEndDate);
	...
	// 					$session_to = $sessionEndDate;
	// 					// if($session_to < $current_date && $ordDtls->public_school_title_status != 'checked'){
	// 					if($session_to < $current_date && $order_schedules_without_public == 1){
	// 						//19-04-2023
	// 						$statusCreateInvoice = $this->App_model->get_status_createInvoice_by_inv_id_range($val['order_id'], $presenter_id, $inv_id->invoice_id,$val['bill_period_start_date'],$sessionEndDate, 'unchecked');

	// 						$no_of_rows = $this->App_model->no_of_rows_by_inv_id_range($val['order_id'], $presenter_id, $inv_id->invoice_id,$val['bill_period_start_date'],$sessionEndDate, 'unchecked');
	...
	// 							// $displayArray['order_id'] = $key;
	// 							$displayArray['order_id'] = $val['order_id'];
	...
	// 								$displayArray['assigned_hours'] = $ordDtls->assigned_hours;
	// 								$displayArray['school_id'] = $ordDtls->school_id;
	// 								$displayArray['schedule_status'] = 'unchecked';
	// 							}else{
	// 								$displayArray['school'] = 'N/A';
	// 								$displayArray['title'] = 'N/A';
	// 								$displayArray['order_no'] = 'N/A';
	...
	// 							$invoicePath_new = $this->App_model->get_invoice_new_by_inv_id($val['order_id'], $inv_id->invoice_id,$presenter_id);
	// 							// print_r($invoicePath); die();
	// 							if(isset($invoicePath_new)){
	// 								$doc = $invoicePath_new->invoice_document;
	// 							}else{
	...
	// 							$displayArray['billing_period'] = $billing_period_s.' to '.$billing_period_e;
	// 							$displayArray['sessionEndDate'] = $sessionEndDate;
	// 							$displayArray['bill_period_start_date'] = $val['bill_period_start_date'];
								

	// 							$displayArray['dwnld_flag'] = $this->App_model->get_download_status_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id);
		
	// 							$displayArray['payment_schedule_flag'] = $paymentScheduleFlag;

	// 							$displayArray['order_schedules'] = $this->App_model->get_order_schedule_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id, "order_schedules.id",$val['bill_period_start_date'],$sessionEndDate);
	...
	// 								}else{
	// 									$displayArray['completed_by'] = 'N/A';
	// 									// $displayArray['completed_by_DateFormat'] = NULL;
	// 								}
	...
	// 								//flag set for download button.
	// 								if($displayArray['dwnld_flag']){
	// 									$displayArray['payment_date'] = $this->App_model->get_payment_date($val['order_id'], $presenter_id, $displayArray['session_from'], $displayArray['session_to']);
	// 								}else{
	// 									$displayArray['payment_date'] = NULL;
	// 								}
	...
	// 								$displayArray['session_to'] = 'N/A';
	// 								// $displayArray['dwnld_flag'] = 'N/A';
	// 								$displayArray['dwnld_flag'] = $this->App_model->get_download_status($val['order_id'], $presenter_id, $val['bill_period_start_date'], $sessionEndDate);
	// 								$displayArray['late_flag'] = 0;
	// 								$displayArray['payDate_flag'] = 0;
	// 								$displayArray['sessionEndDate'] = $sessionEndDate;
	// 							}else{
	...
	// 							}
	// 							// $finalArray[]=$displayArray;
	// 							$finalArrayForPublicSchoolTitle[]=$displayArray;
								
								
	// 						}
	// 					// }else if($check_past_schedules_for_public == 1 && $ordDtls->public_school_title_status == 'checked'){
	// 					}else if($check_past_schedules_for_public == 1 && $order_schedules_with_public == 1){
	// 						//19-04-2023
	// 						$statusCreateInvoice = $this->App_model->get_status_createInvoice_by_inv_id_range($val['order_id'], $presenter_id, $inv_id->invoice_id,$val['bill_period_start_date'],$sessionEndDate, 'checked');

	// 						$no_of_rows = $this->App_model->no_of_rows_by_inv_id_range($val['order_id'], $presenter_id, $inv_id->invoice_id,$val['bill_period_start_date'],$sessionEndDate, 'checked');
	...
	// 						}
	// 						//19-04-2023
	// 						// echo 'checked'.$ordDtls->order_no; echo '</br>';
	// 						if($scheduleHours){
	...
	// 								$displayArray['assigned_hours'] = $ordDtls->assigned_hours;
	// 								$displayArray['school_id'] = $ordDtls->school_id;
	// 								$displayArray['schedule_status'] = 'checked';
	// 							}else{
	// 								$displayArray['school'] = 'N/A';
	// 								$displayArray['title'] = 'N/A';
	// 								$displayArray['order_no'] = 'N/A';
	...
	// 							$displayArray['order_schedules'] = $this->App_model->get_order_schedule_by_inv_id_past($val['order_id'], $presenter_id, $inv_id->invoice_id, "order_schedules.id",$val['bill_period_start_date'],$sessionEndDate);
	...
	// 								$displayArray['session_to'] = 'N/A';
	// 								// $displayArray['dwnld_flag'] = 'N/A';
	// 								$displayArray['dwnld_flag'] = $this->App_model->get_download_status_public($val['order_id'], $presenter_id, $val['bill_period_start_date'], $sessionEndDate,$inv_id->invoice_id);
	// 								$displayArray['late_flag'] = 0;
	// 								$displayArray['payDate_flag'] = 0;
	// 								$displayArray['sessionEndDate'] = $sessionEndDate;
	// 							}else{
	...
	// 						}
	// 					}
	// 					if($order_schedules_with_public == 1 && $order_schedules_without_public == 1){
	...
	// 						if($check_past_schedules_for_public == 1 && $order_schedules_with_public == 1){
	// 							if($scheduleHours){
	...
	// 				}
	// 		}else{
	// 			$finalArrayForPublicSchoolTitle[] = $val;
	// 		}
	// 	}
	...
	// 	$data['orders'] = $finalArrayForPublicSchoolTitle;
    //     $data['page'] = 'presenters';
    //     $data['page_title'] = SITE_NAME.' :: Presenters Management &raquo; Orders';
	...
	public function billing($para = NULL) {
        ...
        $current_date = date("Y-m-d");
        // getting billing rate of the presenter 
        $billRate = $this->App_model->get_bill_rate($presenter_id); //done
        // getting order_ids by presenters
        // $ordersByPresenter = $this->App_model->get_orders_by_presenters($presenter_id);
		// code for session wise data
		$session_id = $this->input->get('pre_blng_session_id');
		if(isset($session_id)){
			// getting order_ids by presenters sessionwise
			$ordersByPresenter = $this->App_model->get_orders_by_presenters_sessionwise($presenter_id, $session_id); //done
			$data['session_id'] = $session_id;
			$data['totHoursAssgnd'] = $this->App_model->get_total_hours_assigned($session_id,$this->session->userdata('id'));
			// print_r($data['totHoursAssgnd']); die();
			$data['totHoursSchedule'] = $this->App_model->get_total_hours_schedule($session_id,$this->session->userdata('id')); //done
			// print_r($data['totHoursSchedule']); die();
		}else{
			$curr_date = date("Y-m-d h:i:s");
			$curr_session_id = $this->App_model->get_curr_session_id($curr_date);
		...
		if(is_array($ordersByPresenter) || is_object($ordersByPresenter)){//added 04-01-2022
			foreach($ordersByPresenter as $row){
				$dates = $this->App_model->get_orderSchedules_by_order_id($row->order_id); //done
				// echo '<pre>'; print_r($dates); die();
				$orderArray[$row->order_id] = array();
				foreach($dates as $dt){
					// array_push($orderArray[$row->order_id],$dt->date.'-01');
        ...
        foreach($orderArray as $key => $value){
			...
			//echo $key.'<br/>';
            // getting order_no,school_id,school_name,title_name,assigned_hours by order_id and presenter
            $ordDtls =  $this->App_model->get_presenter_orders_details($key,$presenter_id); //done
            // echo '<pre>'; print_r($ordDtls); 
            ...
            foreach($value as $val){
                ...
                $flag = date("d", strtotime($val)) == '01' ? 1 : 2;
                // total hours scheduled within session dates
                $scheduleHours = $this->App_model->get_scheduled_hours($key, $presenter_id, $val, $flag); //done
                // echo $scheduleHours; die();
                // total scheduled hours of presenter
                $totalScheduleHours = $this->App_model->get_total_scheduled_hours($key, $presenter_id); //done
                // total scheduled hours within session dates whose status is confirm. i.e after scheduling status should be confirm hours or after that.
                $scheduleHoursConfirm = $this->App_model->get_scheduled_hours_confirm($key, $presenter_id, $val, $flag);  //done
                // getting status of orders within the session dates whose status is 'create invoice'.
                $statusCreateInvoice = $this->App_model->get_status_createInvoice($key, $presenter_id, $val, $flag); //done
                // counting total on of records within session dates.
                $no_of_rows = $this->App_model->no_of_rows($key, $presenter_id, $val, $flag); //done
                // condition to check if submit invoice button be enable or disable.
                if($statusCreateInvoice == $no_of_rows){
                    $submitInvoiceCounter = 1;
                }else{
				...
				$invoice_createdDate = $this->App_model->get_invoice_created_date($key, $val); //done
				if(isset($invoice_createdDate)){
					$created_date = $invoice_createdDate->created_date;
				}else{
					$created_date = 0;
				...
				//check invoice submit before 2021-10-25
				$oldrSubInv = $this->App_model->older_submitted_invoice($key, $presenter_id, $val, $flag); //done
                // getting pshedule_id,session_from,session_to,billing_date,payment_date,invoice_no,email_remonder_date,month,year,is_deleted  by checking 'session from' date
                $othersDetails = $this->App_model->get_billing_date($val);
                ...
                $session_to = $sessionEndDate;
                    ...
                    if($scheduleHours){
                        ...
                        $displayArray['dwnld_flag'] = $this->App_model->get_download_status($key, $presenter_id, $val, $sessionEndDate); //done and from live ihve pasted it on clone

                        $displayArray['payment_schedule_flag'] = $paymentScheduleFlag;
                        $displayArray['order_schedules'] = $this->App_model->get_order_schedule_with_range($key, $presenter_id, $val, $sessionEndDate, "order_schedules.id"); //done
                        ...
                        if($othersDetails){
                            ...
                            //flag set for download button.
                            if($displayArray['dwnld_flag']){
                                $displayArray['payment_date'] = $this->App_model->get_payment_date($key, $presenter_id, $displayArray['session_from'], $displayArray['session_to']);  //done
                            }else{
                                $displayArray['payment_date'] = NULL;
                            }
                            ...
                            $displayArray['session_to'] = 'N/A';
                            // $displayArray['dwnld_flag'] = 'N/A';
                            $displayArray['dwnld_flag'] = $this->App_model->get_download_status($key, $presenter_id, $val, $sessionEndDate); // done previously
                            $displayArray['late_flag'] = 0;
                            $displayArray['payDate_flag'] = 0;
                            $displayArray['sessionEndDate'] = $sessionEndDate;
		...
		foreach($finalArray as $key => $val){
			...
			// print_r($schedules_id);
			// checking unique invoice ids without 0 values
			$check_is_unique_invoice_id = $this->App_model->check_is_unique_invoice_id($schedules_id); //done
			// echo '<pre>'; print_r($check_is_unique_invoice_id);
			// die;

			if($check_is_unique_invoice_id != false){
					// getting order_no,school_id,school_name,title_name,assigned_hours by order_id and presenter
					$ordDtls =  $this->App_model->get_presenter_orders_details($val['order_id'],$presenter_id); //done
					...
					foreach($check_is_unique_invoice_id as $key => $inv_id){
						...
						$invoice_createdDate = $this->App_model->get_invoice_created_date($val['order_id'], $val['bill_period_start_date']); //done
						...
						$oldrSubInv = $this->App_model->older_submitted_invoice_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id);  //done
						// getting pshedule_id,session_from,session_to,billing_date,payment_date,invoice_no,email_remonder_date,month,year,is_deleted  by checking 'session from' date
						$othersDetails = $this->App_model->get_billing_date($val['bill_period_start_date']); //done
						// echo '<pre>'; print_r($othersDetails); die();
						//payement schedule is set or not. 
						...
						$order_schedules_without_public = $this->App_model->get_order_schedule_with_range_with_public($val['order_id'], $presenter_id, $val['bill_period_start_date'],$sessionEndDate,$inv_id->invoice_id,$schedules_id,'0');

						$order_schedules_with_public = $this->App_model->get_order_schedule_with_range_with_public($val['order_id'], $presenter_id, $val['bill_period_start_date'],$sessionEndDate,$inv_id->invoice_id,$schedules_id,'1');

						$check_past_schedules_for_public = $this->App_model->get_order_schedule_with_range_with_public($val['order_id'], $presenter_id, $val['bill_period_start_date'],$sessionEndDate,$inv_id->invoice_id,$schedules_id,'2'); //done
						// echo $check_past_schedules_for_public.$val['order_id'].''.$order_schedules_with_public; echo '</br>'; echo '</br>';
						...
						if($session_to < $current_date && $order_schedules_without_public == 1){
							...
							// total hours scheduled by invoice id
							$scheduleHours = $this->App_model->get_scheduled_hours_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id,$schedules_id,'0'); //done
							
							// total scheduled hours by invoice id whose status is confirm. i.e after scheduling status should be confirm hours or after that.
							$scheduleHoursConfirm = $this->App_model->get_scheduled_hours_confirm_by_inv_id($val['order_id'], $presenter_id,$inv_id->invoice_id,$schedules_id,'0'); //done

							//19-04-2023
							$statusCreateInvoice = $this->App_model->get_status_createInvoice_by_inv_id_range($val['order_id'], $presenter_id, $inv_id->invoice_id,$val['bill_period_start_date'],$sessionEndDate, 'unchecked',$schedules_id); // done ihve paseted from live to clone 

							$no_of_rows = $this->App_model->no_of_rows_by_inv_id_range($val['order_id'], $presenter_id, $inv_id->invoice_id,$val['bill_period_start_date'],$sessionEndDate, 'unchecked',$schedules_id); //done ihve added from live to clone
							...
							//19-04-2023
							// echo $submitInvoiceCounter.$val['order_id']; echo '</br>';
							...
							if($scheduleHours){
								...
								$invoicePath_new = $this->App_model->get_invoice_new_by_inv_id($val['order_id'], $inv_id->invoice_id,$presenter_id); //done
								// print_r($invoicePath); die();
								if(isset($invoicePath_new)){
									$doc = $invoicePath_new->invoice_document;
								}else{
								...
								$displayArray['dwnld_flag'] = $this->App_model->get_download_status_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id);  //done
		
								$displayArray['payment_schedule_flag'] = $paymentScheduleFlag;

								$displayArray['order_schedules'] = $this->App_model->get_order_schedule_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id,$val['bill_period_start_date'],$sessionEndDate,$schedules_id,'0'); //done
								...
								if($othersDetails){
									...
									//flag set for download button.
									if($displayArray['dwnld_flag']){
										$displayArray['payment_date'] = $this->App_model->get_payment_date($val['order_id'], $presenter_id, $displayArray['session_from'], $displayArray['session_to']); //done
									}else{
										$displayArray['payment_date'] = NULL;
									}
									...
									$displayArray['sessionEndDate'] = $sessionEndDate;
									$displayArray['completed_by_DateFormat'] = NULL;
									$displayArray['payment_schedule_flag'] = 1;
								}
								// $finalArray[]=$displayArray;
								$finalArrayForPublicSchoolTitle[]=$displayArray;
						...
						}else if($check_past_schedules_for_public == 1){

							// total hours scheduled by invoice id
							$scheduleHours = $this->App_model->get_scheduled_hours_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id,$schedules_id,'1'); //done
						
							// total scheduled hours by invoice id whose status is confirm. i.e after scheduling status should be confirm hours or after that.
							$scheduleHoursConfirm = $this->App_model->get_scheduled_hours_confirm_by_inv_id($val['order_id'], $presenter_id,$inv_id->invoice_id,$schedules_id,'1'); //done

							//19-04-2023
							$statusCreateInvoice = $this->App_model->get_status_createInvoice_by_inv_id_range($val['order_id'], $presenter_id, $inv_id->invoice_id,$val['bill_period_start_date'],$sessionEndDate, 'checked',$schedules_id);

							$no_of_rows = $this->App_model->no_of_rows_by_inv_id_range($val['order_id'], $presenter_id, $inv_id->invoice_id,$val['bill_period_start_date'],$sessionEndDate, 'checked',$schedules_id);
							...
							if($scheduleHours){
								...
								$displayArray['sessionEndDate'] = $sessionEndDate;
								$displayArray['bill_period_start_date'] = $val['bill_period_start_date'];
								$displayArray['session_from'] = $val['bill_period_start_date'];
								...
								$displayArray['order_schedules'] = $this->App_model->get_order_schedule_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id, $val['bill_period_start_date'],$sessionEndDate,$schedules_id,'2'); //done
								...
								if($othersDetails){
									...
									$displayArray['session_from'] = $othersDetails->session_from;
									$displayArray['session_to'] = $othersDetails->session_to;
									$displayArray['bill_period_start_date'] = $othersDetails->session_from;
						...
						if($order_schedules_with_public == 1 && $order_schedules_without_public == 1){
							...
							if($order_schedules_without_public == 1 && $session_to < $current_date){
								// total hours scheduled by invoice id
								$scheduleHours = $this->App_model->get_scheduled_hours_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id,$schedules_id,'0'); //done
								...
								//19-04-2023
								$statusCreateInvoice = $this->App_model->get_status_createInvoice_by_inv_id_range($val['order_id'], $presenter_id, $inv_id->invoice_id,$val['bill_period_start_date'],$sessionEndDate, 'unchecked',$schedules_id); //done 

								$no_of_rows = $this->App_model->no_of_rows_by_inv_id_range($val['order_id'], $presenter_id, $inv_id->invoice_id,$val['bill_period_start_date'],$sessionEndDate, 'unchecked',$schedules_id); //done
								...
								if($scheduleHours){
									...
									$displayArray['order_schedules'] = $this->App_model->get_order_schedule_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id,$val['bill_period_start_date'],$sessionEndDate,$schedules_id,'0');
							...
							if($check_past_schedules_for_public == 1){
								// total hours scheduled by invoice id
								$scheduleHours = $this->App_model->get_scheduled_hours_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id,$schedules_id,'1');
							
								// total scheduled hours by invoice id whose status is confirm. i.e after scheduling status should be confirm hours or after that.
								$scheduleHoursConfirm = $this->App_model->get_scheduled_hours_confirm_by_inv_id($val['order_id'], $presenter_id,$inv_id->invoice_id,$schedules_id,'1');
								...
								if($scheduleHours){
									...
									$displayArray['order_schedules'] = $this->App_model->get_order_schedule_by_inv_id($val['order_id'], $presenter_id, $inv_id->invoice_id, $val['bill_period_start_date'],$sessionEndDate,$schedules_id,'2');
		...
		$arr = array_unique($finalArrayForPublicSchoolTitle,SORT_REGULAR);
		//session from table
		$data['s_array'] = $this->App_model->get_sessions();
		$data['rdyInvc'] = $para;
        // $data['orders'] = $finalArray;
	...
	public function create_log() {
		$tem_list = $this->App_model->log_template();
		...
		$topic_id=$data['schedule']->topic_id;
        $topic_text = $this->App_model->get_custom_template($topic_id , $presenter_id);
        if($topic_text != false){
        $data['topic_text_final'] = $topic_text[0]->message;
        }
		$data['tem_list']=$tem_list;
		$data['page'] = 'presenters';
	...
	public function logs_old_05_12_22() {
	...
	public function logs() {
        ...
        $actual_link = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
        // $logStatus = parse_url($actual_link, PHP_URL_QUERY); 

        $link_array = explode('/', $actual_link);
        // echo '<pre>'; print_r($link_array); 
        // echo $link_array['7'];
        if(isset($link_array['6'])){
            $logStatus = $link_array['6'];
        }else{
            $logStatus = '';
        }


        if($logStatus == 'submitted_logs'){
            $default_uri = array('page, order_no, school, presenter, topic, date');
            // $uri = $this->uri->uri_to_assoc(4, $default_uri);
            ...
            // $config = $this->init_pagination('app/presenters/logs/'.$this->uri->assoc_to_uri($pegination_uri).'/page/', 15, $total_rows);
            $config = $this->init_pagination('app/presenters/logs/'.$logStatus.'/'.$this->uri->assoc_to_uri($pegination_uri).'/page/', 16, $total_rows);
        ...
        // $tem_list = $this->App_model->custom_tmp(); //get log template 22/11/22
        // $data['tem_list']=$tem_list;
        ...
        if($logStatus == 'log_writing_room' || $logStatus == '' || $logStatus == 'page'){
            $default_uri = array('page');
            if($logStatus == 'log_writing_room'){
                $uri = $this->uri->uri_to_assoc(5, $default_uri);
            }else{
                $uri = $this->uri->uri_to_assoc(4, $default_uri);
            }
            // $uri = $this->uri->uri_to_assoc(4, $default_uri);
            $pegination_uri_logWrite =array();
            $filterLogWritingRm = array();
            ...
            // Get the total rows without limit
            $total_rows_logWrite = $this->App_model->custom_tmp($filterLogWritingRm, true);
            if($logStatus == 'log_writing_room'){
                $config = $this->init_pagination('app/presenters/logs/'.$logStatus.'/'.$this->uri->assoc_to_uri($pegination_uri_logWrite).'/page/', 6, $total_rows_logWrite);
            }else{
                $config = $this->init_pagination('app/presenters/logs/'.$this->uri->assoc_to_uri($pegination_uri_logWrite).'/page/', 5, $total_rows_logWrite);
            }
            // $config = $this->init_pagination('app/presenters/logs/'.$this->uri->assoc_to_uri($pegination_uri_logWrite).'/page/', 5, $total_rows_logWrite);
            ...
            $filterLogWritingRm['limit'] = $config['per_page'];
            $filterLogWritingRm['offset'] = $limit_end;
            $tem_list = $this->App_model->custom_tmp($filterLogWritingRm, false,$this->session->userdata('id')); //get custom template
            // echo '<pre>'; print_r($tem_list);
            $data['tem_list']=$tem_list;
        }

        $data['logStatus'] = $logStatus;  //added new 22-11-22
        // $data['filter'] = $filter;
        $data['page'] = 'presenters';
        $data['page_title'] = SITE_NAME.' :: Presenters Management';
	...
	public function create_log_billing() {
        ...
        if($topic_text != false){
        // $data['topic_text_final'] = $topic_text[0]->message;
		$data['topic_text_final'] = html_entity_decode($topic_text[0]->message, ENT_QUOTES, 'UTF-8');
        }
        $data['page'] = 'presenters';
        $data['page_title'] = SITE_NAME.' :: Presenters Management &raquo; Create Log';
	...
	function request_teacher_modal(){
		$is_grade_present = $this->input->post('is_grade_present');
		if(empty($is_grade_present)){
			$grade_id = $this->input->post('grade');
			$grade_name = $this->App_model->get_grade_name($grade_id);
			$school_id = $this->input->post('schlid');
			$title_id = $this->input->post('titleid');
			$data['is_grade_present'] = $is_grade_present;
			$data['teacher_grades']=$this->App_model->get_teacher_grades_title($school_id,$title_id);
			$data['grade_name'] = $grade_name;
			echo $this->load->view('presenters/view_request_teacher_modal',$data, true);
		}else{
	...
	public function export_teacher_report_from_provider(){
        
        $presenter_id= $this->session->userdata('id');
        $results = $this->App_model->export_teacher_report_from_provider($presenter_id);
        $delimiter = ",";
        ...
        foreach ($results as $val) {
            // $lineData = array($val['session_id'], html_entity_decode($val['school_name'], ENT_QUOTES, 'UTF-8'), $val['created_by'], $val['teacher'], $val['grade_id'], $val['topic_id'],$val['total_hours'], $val['start_date']);

			$lineData = array(html_entity_decode($val['session_id'], ENT_QUOTES, 'UTF-8'), html_entity_decode($val['school_name'], ENT_QUOTES, 'UTF-8'), html_entity_decode($val['created_by'], ENT_QUOTES, 'UTF-8'), html_entity_decode($val['teacher'], ENT_QUOTES, 'UTF-8'), html_entity_decode($val['grade_id'], ENT_QUOTES, 'UTF-8'), html_entity_decode($val['topic_id'], ENT_QUOTES, 'UTF-8'),$val['total_hours'], $val['start_date']);
	...
	public function log_template_for_presenters(){
		...
		$data['tem_list']=$tem_list;
		$data['main_content'] = 'presenters/log_template_for_presenters';
		$this->load->view(TEMPLATE_PATH, $data);
	}
	...
	public function add_log_template(){
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST')
		{
			...
			if ($this->form_validation->run())
			{
				...
				if ($insert_id = $this->App_model->insert('presenters_log', $data)){
  
  
					$this->session->set_flashdata('message_type', 'success');
					$this->session->set_flashdata('message', '<strong>Well done!</strong> The template has been added successfully. ');
					redirect('/app/presenters/log_template_for_presenters');
				}else{
					$this->session->set_flashdata('message_type', 'danger');
					$this->session->set_flashdata('message', '<strong>Oh snap!</strong> Change something and try again.');
					redirect('/app/presenters/log_template_for_presenters');
		...
		$data['main_content'] = 'presenters/log_template';
		$this->load->view(TEMPLATE_PATH, $data);
	}

	public function delete_log_template($id) {

		$this->App_model->delete('presenters_log',$id);
		$this->session->set_flashdata('message_type', 'success');
		$this->session->set_flashdata('message', '<strong>Well done!</strong> The template has been successfully deleted. ');
		redirect('/app/presenters/log_template_for_presenters');
	}

	public function edit_log_template($id){
        // echo $id;
        if ($this->input->server('REQUEST_METHOD') === 'POST'){
            $this->form_validation->set_rules('tmp_name', 'Subject', 'trim|required');
            $this->form_validation->set_rules('message', 'Message', 'trim|required');
            if ($this->form_validation->run())
            {
                ...
                $this->App_model->update('presenters_log', 'id', $id, $data);
				$this->session->set_flashdata('message_type', 'success');
            ...
            $presenter_log_details=$this->App_model->get_presenter_log($id);
            // echo '<pre>'; print_r($presenter_log_details); die;
            $data['log_content']=$presenter_log_details;

            $data['main_content'] = 'presenters/edit_template';
            $this->load->view(TEMPLATE_PATH, $data);
        }
    }

	public function log_writting_room() {

        $tem_list = $this->App_model->custom_tmp();
        // echo '<pre>'; print_r($tem_list); die;
        // $topic_names = $this->App_model->topic_name();
        // echo '<pre>'; print_r($topic_name); die;
         $data['tem_list']=$tem_list;
        // $data['topic_names'] = $topic_names;

        $data['main_content'] = 'presenters/log_writting_room';
        $this->load->view(TEMPLATE_PATH, $data);
    }

	public function custom_template_for_admin(){
        $id = $this->session->userdata('id');
        $result = $this->App_model->get_topic($id);
        $template_topic_id = $this->App_model->get_topic_from_custom_template($id);
		// echo '<pre>'; print_r($template_topic_id);
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
       {
           ...
           if ($this->form_validation->run())
           {
               ...
               if ( $this->App_model->insert('custom_templates', $data)){
 
 
                   $this->session->set_flashdata('message_type', 'success');
                   $this->session->set_flashdata('message', '<strong>Well done!</strong> The template has been added successfully.');
                   redirect('/app/presenters/logs');
               }else{
                   $this->session->set_flashdata('message_type', 'danger');
                   $this->session->set_flashdata('message', '<strong>Oh snap!</strong> Change something and try again.');
                   redirect('/app/presenters/logs');
        ...
        $data['template_topic_id']=$template_topic_id;
         $data['topic']=$result;
        $data['main_content'] = 'presenters/custom_template_for_admin';
        $this->load->view(TEMPLATE_PATH, $data);
    }

	public function custom_template_edit($id){
		$presenter_id = $this->session->userdata('id');
        $result = $this->App_model->get_topic($presenter_id);
        $template_topic_id = $this->App_model->get_topic_from_custom_template($presenter_id);
        if ($this->input->server('REQUEST_METHOD') === 'POST'){
			$this->form_validation->set_rules('topic', 'Topic', 'trim|required');
            $this->form_validation->set_rules('message', 'Message', 'trim|required');
            if ($this->form_validation->run())
            {
                ...
                $this->App_model->update('custom_templates', 'id', $id, $data);
            ...
            $presenter_log_details=$this->App_model->custom_template_log($id);
            $data['log_content']=$presenter_log_details;
			$data['topic']=$result;
            $data['template_topic_id']=$template_topic_id;

            $data['main_content'] = 'presenters/custom_template_edit';
            $this->load->view(TEMPLATE_PATH, $data);
        }
	...
	public function custom_template_delete($id) {
    
         $this->App_model->delete('custom_templates',$id);
         $this->session->set_flashdata('message_type', 'success');
         $this->session->set_flashdata('message', '<strong>Well done!</strong> The template has been successfully deleted.');
         redirect('/app/presenters/logs');
    }

	public function favorite_template($id=null) {
		$presenter_id = $this->session->userdata('id');
		$favorite_template = $this->App_model->favorite_template($presenter_id);
		...
		if($id == null){
			if ($this->input->server('REQUEST_METHOD') === 'POST')
			{
				...
				if ($this->form_validation->run()){
					...
					if ( $this->App_model->insert('favorite_template', $data)){
		
		
						$this->session->set_flashdata('message_type', 'success');
						$this->session->set_flashdata('message', '<strong>Well done!</strong> The template has been added successfully.');
						redirect('/app/presenters/favorite_template');
					}else{
						$this->session->set_flashdata('message_type', 'danger');
						$this->session->set_flashdata('message', '<strong>Oh snap!</strong> Change something and try again.');
						redirect('/app/presenters/favorite_template');
			...
			if($favorite_template != false){
				$data['favorite_template']=$favorite_template;
			}
			$data['main_content'] = 'presenters/favorite_template';
			$this->load->view(TEMPLATE_PATH, $data);
		}

	}

	public function favorite_template_delete($id) {
        // echo 'hii';
        $this->App_model->delete('favorite_template',$id);
        $this->session->set_flashdata('message_type', 'success');
        $this->session->set_flashdata('message', '<strong>Well done!</strong> The template has been successfully deleted.');
        redirect('/app/presenters/favorite_template');
    }

	public function favorite_template_edit($id) {
        $message = $this->input->post('message');
                $data = array(
                    'message' => $message,
                );
        $this->App_model->update('favorite_template', 'id', $id, $data);
	...
	public function viewTemplateModal(){
        ...
        $message = $this->App_model->get_template_message($template_id);
	...
	// code for suggestive logs phase5
	public function use_custom_template(){
        $id = $this->session->userdata('id');
        $topic_id = $this->input->post('topic_id');
        $topic_text = $this->App_model->get_custom_template($topic_id , $id);
        if($topic_text == false){
            echo false;
        }else{
            $custom_templates_val =  $topic_text[0]->message;
            echo $custom_templates_val;
        }
    }

    public function use_favorite_template(){
        $id = $this->session->userdata('id');
        $favorite_template = $this->App_model->get_favorite_template($id);
        if($favorite_template == false){
            echo false;
        }else{
			$favorite_template_val =  $favorite_template->message;
			echo $favorite_template_val;
        }

    }

    public function use_library_topic(){
        $topic_id = $this->input->post('topic_id');
        // $library_topic_text = $this->App_model->get_library_topic($topic_id);
		$library_topic_text = $this->App_model->get_admin_library_topic($topic_id);
		
        if($library_topic_text == false){
            echo false;
        }else{
            $library_topic_text =  $library_topic_text->description;
            echo $library_topic_text;
        }

    }

	public function brienza_template(){
        $brienza_template_id = $this->input->post('brienza_template_id');
        $brienza_template = $this->App_model->get_brienza_template($brienza_template_id);

		// added for new design
		// $this->session->set_userdata('brienza_template_selected', '');
		$options = '';
		$tem_list = $this->App_model->log_template();
		foreach($tem_list as $item){
			if($item->id == $brienza_template_id){
				$options .='<li class="selected" id="deselect">
				<a href="#" style="font-size: 14px;font-family:Ubuntu, sans-serif;text-decoration: none; color: white;" id="'.$item->id.'" >'.$item->tmp_name.'</a>
			</li>';
			}else{
				$options .='<li>
				<a href="#" style="font-size: 14px;font-family:Ubuntu, sans-serif; text-decoration: none; color: white;" id="'.$item->id.'" >'.$item->tmp_name.'</a>
		...
		// end of added for new design

        if($brienza_template == false){
            echo false;
        }else{
			$brienza_template_val =  $brienza_template->message;
			$final = new stdClass();
			$final->val = $brienza_template_val;
    ...
    //     $topic_id = $this->input->post('topic_id');
    //     $description_val = $this->input->post('description_val');
	// 	$data = array(
	// 		'message' => $description_val,
	// 	);
    //     $this->App_model->update('custom_templates', 'topic', $topic_id, $data);
	...
	function save_custom_template(){
        ...
        if($topic_text == false){
            ...
            $message_insert = $this->App_model->insert('custom_templates', $data);
			if($message_insert == false){
                echo '0';
            }
            else{
            ...
            $message_update = $this->App_model->update('custom_templates', 'topic', $topic_id, $data);
			if($message_update == false){
                echo '0';
            }
            else{
                echo $description_val;
     ...
     function print_log(){
        ...
        $log = $this->App_model->print_log($log_id);
        // $log_content = $log->content_for_print;
        // echo $log_content;
		if (isset($log->content_for_print) && $log->content_for_print != NULL){
			$log_content = $log->content_for_print;
			echo $log_content;
		   }else{
	...
	public function export_presenters(){
		// $filter['presenter'] = isset($_GET['presenter']) ? $_GET['presenter'] : '';
		// $filter['company'] = isset($_GET['company']) ? $_GET['company'] : '';
		// $filter['email'] = isset($_GET['email']) ? $_GET['email'] : '';
		$search_box = isset($_GET['search_box']) ? $_GET['search_box'] : '';
		if($search_box == ''){
			$filter['presenter'] = isset($_GET['presenter']) ? $_GET['presenter'] : '';
			$filter['company'] = isset($_GET['company']) ? $_GET['company'] : '';
		}else{
			$filter['search_box'] = isset($_GET['search_box']) ? $_GET['search_box'] : '';
		}
		$filter['role_token'] = 'teacher';
		$filter['deleted'] = 0;
		$results = $this->Admin_model->get_users_list($filter, 'id', 'asc');
        $delimiter = ",";
        $filename = "presenters_list" . date('YmdHis') . ".csv";
        
        //create a file pointer
        $f = fopen('php://memory', 'w');
        
        //set column headers
        $fields = array('Name', 'Email', 'Rate', 'Phone', 'Address');
        fputcsv($f, $fields, $delimiter);
        
        //output each row of the data, format line as csv and write to file pointer
        //  for CSV
        foreach ($results as $key => $val) {
			if(!isset($val->meta['rate'])){
				$rate = 'N/A';
			}else{
				$rate = $val->meta['rate'];
			} 
			if(!isset($val->meta['phone'])){
				$phone = 'N/A';
			}else if(is_numeric($val->meta['phone'])){
				$phone = $val->meta['phone'];
			}else{
				$phone = '';
			} 

			if(!isset($val->meta['address'])){
				$address = 'N/A';
			}else{
				$address = $val->meta['address'];
			} 

            // $lineData = array($val->first_name, $val->email, $rate, $phone, $address);
			$lineData = array(html_entity_decode($val->first_name, ENT_QUOTES, 'UTF-8'), html_entity_decode($val->email, ENT_QUOTES, 'UTF-8'), html_entity_decode($rate, ENT_QUOTES, 'UTF-8'), html_entity_decode($phone, ENT_QUOTES, 'UTF-8'), html_entity_decode($address, ENT_QUOTES, 'UTF-8'));
            fputcsv($f, $lineData, $delimiter);
        }
