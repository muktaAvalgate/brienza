...
require_once APPPATH . "libraries/fpdi/src/autoload.php"; // 12-06-2023
   ...
   private function format_date($date) {
    ...
    function billingProcess(){
        ...
        foreach ($scheduled_ids  as $row) {
            ...
            // echo '<pre>';print_r($pdf1);
            // print_r($pdf1[0]->attachment); die();
...
// echo '<pre>';print_r($documents);die;
                ...
                // echo '<pre>';print_r($pdf2);die;
                if($pdf2){
                $idsOFAttachments[$pdf2[0]->school_pdf]=$pdf2[0]->id;
                // echo '<pre>';print_r($pdf2->school_pdf);die;
        ...
        if(!empty($returnedFiles) &&  is_array($returnedFiles))
        {
            ...
            foreach($returnedFiles as $file){
                ...
                $file=basename($file);
                $id=$idsOFAttachments[$file];
                $dta['pdf_merge_status']=0;
                if($this->App_model->update('order_schedule_status_log', 'id', $id, $dta)){
                    continue;
                }else{
                    return;
                }
            ...
            // echo '<pre>';print('else of returnedfiles');die;
            if($this->App_model->update('billing', 'id', $id, $data)){
                	return 1;
                }else{
                	return 0;
    ...
    function imagetopdf($imagefile){
        ...
        // $file= $_SERVER['DOCUMENT_ROOT'] .DIR_TEACHER_FILES.$imagefile;
	...
	function wordtopdf($wordfile){
		...
		//server
		$contents=$objReader->load($_SERVER['DOCUMENT_ROOT'] .DIR_TEACHER_FILES.$wordfile);
...
try {
		...
		// echo '<pre>';print_r($save_file);die;
		return $save_file;
} catch (Exception $e) {
            // echo '<pre>';print_r($e);die;
            // echo '<pre>';print('exception occured');die;
            return '' ;
        }
	}
    ...
    public function reUpload_document_for_presenter() {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            $log_id = $this->input->post('log_id');
            $order_schedule_id = $this->input->post('row_id');
            $sign_log_status = $this->input->post('sign_log_status'); // 12-07-2023
  ...
  /*  public function reUpload_document_for_presenter() {
    ...
    public function compressPdf() {
        // echo '<pre>';print('called');die;
        // $attachment = (isset($_FILES['file'])?$_FILES['file']:"");
            $log_id = $this->input->post('fileId');
            $attachment=$this->input->post('fileName');
            // echo '<pre>';print($attachment);die;
            // $count=array();
            // $count=explode(",",$attachment);
            
            $existingFile=$_SERVER['DOCUMENT_ROOT'].DIR_TEACHER_FILES.$attachment;
            // 
            if(file_exists($existingFile)){
                
            $existingFilePath=$_SERVER['DOCUMENT_ROOT'].DIR_TEACHER_FILES.$attachment;
            // echo '<pre>';print($existingFilePath);die;      
                        $myTask =  new CompressTask('project_public_3cf9bfa33fcd77a78796ecf0d07ccd64_yjQME30f44972af9fd570cca86ae808112d4e','secret_key_01ace40f2a9319cb641a746f60931cc5_vRQ9x26c507361119eb0dfb032bf70483169c');
                    
                        $file = $myTask->addFile($existingFilePath);
                        // $newName=$attachment.'compressed';
                        $myTask->setCompressionLevel('extreme');
                        // $myTask->setOutputFilename($newName);
                        $myTask->execute();
                        // echo '<pre>';print($existingFile);die;
                        $myTask->download($_SERVER['DOCUMENT_ROOT'].DIR_TEACHER_FILES);
                        // $myTask->download();
    ...
    //         if (!isset($unique_attachments[$order_id])) {
    //             $unique_attachments[$order_id] = []; 
    //             $attachmentCounts[$order_id] = 0; 
    //         }
    //         if (!in_array($attachment_name, $unique_attachments[$order_id])) {
    //             $unique_attachments[$order_id][] = $attachment_name;
    //             $attachmentCounts[$order_id]++; 
    //         }
    //     }
    
    //     // echo '<pre>';
    //     // print_r($attachmentCounts);
    //     // die;
    ...
    public function billing_merge() {
        ...
        foreach ($data['pdfmerge_data'] as $k) {
            ...
            if (!isset($attachmentCounts[$order_id])) {
                $attachmentCounts[$order_id] = 0; 
            }
        ...
        $unique_order_ids = array_unique(array_column($data['pdfmerge_data'], 'order_id'));
    ...
    function margePdfAfterCompress($files){
        ...
        return $protectedFiles;
            ...
            $new_file = md5(time().rand(401,500)) .'.pdf';
            $pdf->merge('file', APPPATH . '../assets/teachers/'.$new_file);
            // $pdf->merge('file', $_SERVER['DOCUMENT_ROOT'].DIR_PDF_UPLOADED_FILES.$new_file);
            }else {
                $new_file = '';
            }
            return DIR_TEACHER_FILES.$new_file;
