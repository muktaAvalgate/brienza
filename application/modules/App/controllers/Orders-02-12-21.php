...
class Orders extends Application_Controller {
    ...
    **/
	...
	public function billing() {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            // echo $payemntFlag; echo " -P";
            // echo $completeFlag; echo " -C";
            // echo $approveFlag; echo " -A";
            // die();
            ...
            foreach ($status as $order_schedule_id => $stat) {
                ...
                if($status[$order_schedule_id] == 'Confirm hours'){
                    ...
                    if ($status[$order_schedule_id] == "Log sent - awaiting principal signature") {
                        ...
                        $data['content'] = '<table width="50%" cellpadding="5" cellspacing="0" border="0" style="border:solid 1px; font-family:\'Ubuntu\', sans-serif;">
                            ...
                            <tr>
                                <td align="center" colspan="2" style="height:40px;"><strong>Presenter\'s Name:</strong> '. $schedule->teacher.'</td>
                            </tr>
                            <tr>
                ...
                // // if (!empty($attachment['name'][$order_schedule_id])) {
                // if (!empty($attachment['name'][$order_schedule_id]) && $flag == 1) {
    ...
    //                     redirect('/app/orders/billing/?order_id='.$order_id);               
    //                 }
    //         }else if(!empty($attachment) && ($approveFlag > 0 || $payemntFlag > 0)){
    ...
    //             foreach($status as $order_schedule_id => $stat){
    //                 if($status[$order_schedule_id] == 'Draft attached' && !empty($attachment['name'][$order_schedule_id]) && $flag == 1){
    //                     $config['upload_path'] = DIR_TEACHER_FILES;
    //                     $config['max_size'] = '25000';
    //                     $config['allowed_types'] = 'jpg|png|jpeg|pdf|doc|docx';
    //                     $config['overwrite'] = FALSE;
    ...
    //                 if($status[$order_schedule_id] == 'Approved' && !empty($attachment['name'][$order_schedule_id]) && $flag == 1){
    //                     $config['upload_path'] = DIR_TEACHER_FILES;
    //                     $config['max_size'] = '25000';
    //                     $config['allowed_types'] = 'jpg|png|jpeg|pdf|doc|docx';
    //                     $config['overwrite'] = FALSE;
	...
	// 							</tr>
	// 							<tr>
	// 								<td align="center" colspan="2" style="height:40px;"><strong>Presenter:</strong> '. $schedule->first_name." ".$schedule->last_name.'</td>
	// 							</tr>
	// 							<tr>
	// 								<td align="center" colspan="2" style="height:40px;"><strong>Date:</strong>'. date_display($schedule->start_date, "l, F j, Y").'</td>
	// 							</tr>
   ...
   private function format_date($date) {
	...
	public function billings(){
		...
		$purchase_order_no='';
		$billing_due_date='';
		$billings = $this->App_model->getAdminBilling($presenter,$billing_due_date,$purchase_order_no,20,$this->uri->segment(4));
		$count= $this->App_model->getAdminBillingcount($presenter,$billing_due_date,$purchase_order_no);
		if ($this->input->server('REQUEST_METHOD') === 'POST')
    	{
