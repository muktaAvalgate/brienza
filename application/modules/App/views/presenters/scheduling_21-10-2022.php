...
<div class="container clearfix">
    <div class="main-content">
        ...
        <div class="row">
			...
			<div class="col-lg-3 col-md-4 text-right" id="old_div">
				<div class="panel panel-success">
					<div class="panel-heading">
	...
	<div class="modal fade" id="adminMessageModalBlink" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                ...
                <div class="modal-body requestTeacher">
                </div>
                <div class="modal-footer">
                    <input type="hidden" name="type" value="admin" />
                    <input type="hidden" name="subject" value="Issue reported from portal" />
                    <button type="button" onclick="requestTeacher();" class="btn btn-primary">Send</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
	...
	function openTeacherRequestModal(){
		...
		// }
		// jQuery.ajax({
		// 	url: '<?php //echo base_url(); ?>App/Presenters/request_teacher_modal',
		// 	data: { grade:grade},
		// 	type: 'post',
		// 	success: function(response){
		// 		$('.requestTeacher').html(response);
		// 		$('#adminMessageModalBlink').modal('show');
		// 	}
		// });


		var is_grade_present = $("#is_grade_present").val();
		if(is_grade_present ==''){
			var grade = $("#grades").val();
			...
			jQuery.ajax({
				url: '<?php echo base_url(); ?>App/Presenters/request_teacher_modal',
				data: { grade:grade, schlid:schlid, titleid:titleid},
				type: 'post',
				success: function(response){
					$('.requestTeacher').html(response);
					$('#adminMessageModalBlink').modal('show');
	...
	function requestTeacher(){
		...
		if(teachername != '' && grade_name != ''){

			// remove_specl_char_teachr = teachername.replace(/[^a-zA-Z ]/g, "");
			// teacher_name_edited = remove_specl_char_teachr.replace(/\s+/g, '_');
			// remove_specl_char_grade = grade_name.replace(/[^a-zA-Z ]/g, "");
			// grade_name_edited = remove_specl_char_grade.replace(/\s+/g, '_');

			// if(teacher_name_edited != '' && grade_name_edited != ''){
			// 	window.location.href=base_url+'app/notifications/request_new_teacher/'+teacher_name_edited+'/'+grade_name_edited;
			// }else{
			// 	alert('Please enter the teacher\'s name and grade properly!');
			// }

			var t_name = btoa(teachername);
			var g_name = btoa(grade_name);
			
			// alert(gname);
			if(t_name != '' && g_name != ''){
				window.location.href=base_url+'app/notifications/request_new_teacher/'+t_name+'/'+g_name;
			}else{
				alert('Please enter the teacher\'s name and grade properly!');
			}
			...
			if(teachername == '' && grade_name == ''){
				alert('Please enter the teacher\'s name and grade!');
			}else if(teachername == ''){
