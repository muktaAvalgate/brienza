    ...
    <div class="main-content">
		...
		<div class="row">
			<div class="col-lg-9 col-md-8">
				...
				<div class="form-group">
					<?php if(isset($order->id)) :?>
						<p class="form-control-static highlight-block-purple"> Title: <?php echo $title_name->name;?> </p>
					<?php endif;?>
				</div>
			</div>
				...
				<table class="table table-striped table-bordered table-hover dataTable no-footer dtr-inline" id="datatable-scheduling"  style="width: 100%;">
					...
					<tbody>
						<?php foreach ($schedules as $row) {?>
							<input type="hidden" id="statusForSave" value="<?php echo $row->status; ?>">
						<tr class="gradeA odd" role="row" data-id="<?php echo $row->id; ?>"data-school-id="<?php echo $row->school_id; ?>" data-order-id="<?php echo $row->order_id;?>" data-status="<?php echo $row->status;?>">
							<td>
					    	...
					    	<td>
								...
								<input type="hidden" data-id="<?php echo $row->id;?>" id="apprvTchrVal<?php echo $row->id; ?>" value="<?php echo $row->teacher;?>">
								  <select name="teachers1"  class="editInput  form-control grade_tech<?php echo $row->id; ?>" data-id="<?php echo $row->id;?>" style="display: none;" >
							...
							<td>
	                            <div class="btn-group btn-group-sm">
                                    ...
                                    ?>
                                        <button type="button" class="btn btn-sm btn-default editBtn " data-id="<?php echo $row->id; ?>" data-name="<?php echo $row->grade_id;?>" data-s="<?php echo $row->school_id; ?>" data-order-value="<?php echo $row->status ?>" style="float: none;"><span class="glyphicon glyphicon-pencil"></span></button>
                                    <?php
                                        }
                                    ?>
	                            </div>
						...
						<tr class="gradeA odd" role="row">
							...
							<td>
								...
								<!-- <input type="text"  value="" class="form-control inputteacher" placeholder="" onclick="blockFreeTextFromSingle();" name="teachers" required readonly> -->
								<select name="teachers1"  class="form-control teach" style="display: none;">
									</div><option value="<?php ;?>">select</option>
								</select>

								<input type="text"  value="" class="form-control inputteacher" placeholder="" name="teachers" required readonly>
							</td>
							<td class="col-md-1">
		...
		<?php endif;?>
			...
			<!-- <div class="col-lg-3 col-md-4 text-right">
				...
				If you would like to add a teacher <a href="#" onclick="openTeacherRequestModal()" data-toggle="modal" data-target="">Click here</a>
                    </div>
				</div>
			</div> -->

			<?php if(isset($selected_school) && isset($selected_order) && $remaining_schedule_hrs != 0){ ?>
			...
			<?php }else{ ?>
				<!-- <div class="col-lg-3 col-md-4 text-right" style="margin-left: 861px;"> -->
				<div class="row" style="left: -9px;position: relative;">
					<div class="col-lg-4 col-md-5">
					</div>
					<div class="col-lg-5 col-md-3">
					</div>
...
$(document).ready(function(){
    $('.editBtn').on('click',function(){
    	 ...
    	 var tid = $(this).attr('data-id'); 
		  ...
		  var data_status = $(this).attr('data-order-value');
		  if(data_status == 'Approved'){
			// alert(tid);
			$(this).closest("tr").find(".editSpan").hide();
			$(this).closest("tr").find(".editBtn").hide();
			// $(this).closest("tr").find(".editInput.grade_teach"+tid).show().prop('disabled',false);
			$(this).closest("tr").find(".grade_tech"+tid).remove();
			// $(this).closest("tr").find(".op").css('display','none');
        	$(this).closest("tr").find(".input_grade"+tid).val($('#apprvTchrVal'+tid).val());
			$(this).closest("tr").find(".editInput").show().prop('disabled',true);
			$(this).closest("tr").find(".editInput.date").show().prop('disabled',false);
    ...
    $('.saveBtn').on('click',function(){
		...
		var data_status = trObj.attr('data-status');
		...
		if(data_status == 'Approved'){
	        ...
	        $.ajax({
	            ...
	            dataType: "json",
	            data:{status:'approved',order_schedule_id:order_schedule_id,updated_date:dt},
...
<script>
	  	  ...
	  	  $.ajax({
        		...
        		success: function(res){
        			if(res.msg == 'multiple')
        			{
        				//lert('kjbkjb');
        				$(".teach").show();
						// $(".teach_hideNew").show();//06-12-2022
        				$(".inputteacher").hide();
        				$(".inputteacher").val('');
        				$(".teach").html(res.opt);
						// $(".teach").hide(); //06-12-2022
        			}else{
 ...
 //    $('.grade_tech').on( "change", function(e) {
	...
	// function blockFreeTextFromSingle() {
	// 	if (confirm("Don't see the teacher that you want? Make sure to send the teacher's name and grade to your PD admin.")) {
	// 	$(".teach_hideNew").hide();
	// 	$(".teach").hide();
	// 	$(".inputteacher").show();
