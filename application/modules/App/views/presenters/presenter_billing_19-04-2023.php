                                                            ...
                                                            <div id="print_log">
                                                                ...
                                                                <!-- <input type="file" name="attachment[<?php echo $row->id;?>]" class="upload_file" > -->
                                                                    ...
                                                                    <div class="fileUpload btn btn-sm btn-primary" style=" padding: 2px 10px;margin-top: 3px; margin-left: 0rem;">
                                                                        ...
                                                                        <input id="photo_log<?php echo $orderSchedules->order_schedule_status_id;?>" type="file" class="upload" onchange="upload_document_for_presenter('<?php echo $orderSchedules->order_schedule_status_id;?>','<?php echo $orderSchedules->id;?>');"/>
                                                                    </div>
                                                                <!-- For update attach file -->
...
<script type="text/javascript">
	...
	function sign(order_id,session_from,session_end,late_flag,completed_by_DateFormat){
...
function declineSchedule(schedule_id){
			...
			onrendered: function (canvas) {
				...
				jQuery.ajax({
					...
					success: function (response) {
						...
						save_invoice_for_biling(order_id,session_from,session_end,late_flag,hdn_completed_by_DateFormat,response.file_name);
					}
				});
			}
		});
	...
	function save_invoice_for_biling(order_id, session_from, session_end, late_flag, hdn_completed_by_DateFormat, content ){
		//alert(content);
		jQuery.ajax({
			url: base_url+'app/orders/save_billing_from_billing',
			data: { order_id:order_id, session_from:session_from, session_end:session_end, late_flag:late_flag, hdn_completed_by_DateFormat:hdn_completed_by_DateFormat, content:content, ajaxCall:true },
			type: 'post',
			async: false,
			success: function (response) {
	...
	function upload_document_for_presenter(log_id,row_id){
