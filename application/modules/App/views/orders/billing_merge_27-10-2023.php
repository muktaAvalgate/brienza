                ...
                <html>
                    ...
                    <?php foreach ($pdfmerge_data as $row) { ?> 
                            <div class="row">
                                <thead>
                                <?php
                                    if(!empty($row->billing_id)  && $lastOrderId != $row->billing_id){ 
                                        $lastAttachmentName = ''; ?>
                                    <tr role="row">
                                        ...
                                        <th><?php if($row->is_merged==null){ 
                                                if($attachmentCount[$row->billing_id] == 0){
                                            ?>
                                             <button type="button" id="mergeBtn.<?php echo $row->billing_id; ?>" class="btn subbtn" data-toggle="modal" data-target="#signModal" onclick="mergePdf('<?php echo $row->billing_id; ?>','<?php echo $row->billing_id; ?>')" >Merge</button>
                                             <?php } else{?>
                                                <button type="button" id="mergeBtn.<?php echo $row->billing_id; ?>" class="btn subbtn" data-toggle="modal" data-target="#signModal" onclick="mergePdf('<?php echo $row->billing_id; ?>','<?php echo $row->billing_id; ?>')" disabled>Merge</button>
                                            <?php } ?>
                                            </th>
                                    </tr>
                                    <?php
                                        }else{ ?>
                                            <button type="button" class="btn subbtn" >Merged Documents Ported To Server</button>
                                     <?php   }
                                    } ?>
                                ...
                                <tbody>
                                        ...
                                        <?php if($row->is_merged==null){ ?>
                                            ...
                                            ?>
                                            <td><button type="button" id="compressBtn.<?php echo $row->id; ?>"  class="btn subbtn " data-toggle="modal" data-target="#signModal" onclick="compressPdf('<?php echo $row->id; ?>','<?php echo $row->attachment; ?>','<?php echo $row->billing_id; ?>','<?php echo $attachmentCount[$row->billing_id]; ?>')" disabled>Ready To Merge</button></td>
                                            <?php } else { ?>
                                                <td><button type="button" id="compressBtn.<?php echo $row->id; ?>"  class="btn subbtn " data-toggle="modal" data-target="#signModal" onclick="compressPdf('<?php echo $row->id; ?>','<?php echo $row->attachment; ?>','<?php echo $row->billing_id; ?>','<?php echo $attachmentCount[$row->billing_id]; ?>')">Convert To Merge</button></td>
                                            <?php } ?>
                                        </tr>
                                    <?php 
                                        }
                                     $lastAttachmentName= $row->attachment;
                                     $lastOrderId= $row->billing_id;
