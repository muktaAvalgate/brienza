...
<div class="container clearfix" id="div_body_for_print" style="height:0px!important;">
					...
					<?php if($row->status == "Log sent - awaiting principal signature" && $this->session->userdata('role') == 'teacher') {?>
                        <div id="print_log">
                            ...
                            <div class="fileUpload btn btn-sm btn-primary" style=" padding: 2px 10px;margin-top: 3px;">
								...
								<input id="photo_log<?php echo $row->order_schedule_status_id;?>" type="file" class="upload" onchange="upload_document_for_presenter('<?php echo $row->order_schedule_status_id;?>','<?php echo $row->id;?>');"/>
                        	</div>
                        </div>
                    <?php }?>
