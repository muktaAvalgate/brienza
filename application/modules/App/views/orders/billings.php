...
<!-- <button type="button"  class="btn btn-info btn-s" onclick="window.location.href = '<?//php echo base_url('app/orders/billing_merge/') ?>'">button</button> -->
	<?php
				...
				<td class="sorting_1" style="padding-right: 8px;width: 180px;"><?php echo $item->order_no;?>
	            	...
	            	<td>
	            		<?php if($item->process == 0){?>
	            			<button class="btn btn-success btn-xs" id="processBtn_<?php echo $item->id ?>" onclick="billingProcess(<?php echo $item->id?>,<?php echo $item->order_id?>)">Process</button>

							<button type="button" class="btn btn-success btn-xs" id="spinnerBtn_<?php echo $item->id ?>" style="display:none;">Please wait<i class="fa fa-spinner fa-spin" style="margin-left: 5px;"></i></button>
							<?php }elseif($item->process == 2){ ?>
								<button class="btn btn-success btn-xs" id="processBtn_<?php echo $item->id ?>" onclick="billingProcess(<?php echo $item->id?>,<?php echo $item->order_id?>)" disabled>Processing</button>
							<?php }else{?>
...
<script type="text/javascript">
	...
	function billingProcess(id,order_id){
		...
		// $('#processBtn_'+id).attr('disabled', 'disabled');
		$('#processBtn_'+id).css("display","none");
		$('#spinnerBtn_'+id).css("display","block");
		// alert('click 2');
		var delay = 1000;
		setTimeout(function() {
		// your code
		billingProcess_by(id,order_id);
		}, delay);
	}

	function billingProcess_by(id,order_id){
		// $('#processBtn_'+id).attr('disabled', 'disabled');
		$('#processBtn_'+id).css("display","none");
		$('#spinnerBtn_'+id).css("display","block");
		jQuery.ajax({
