    ...
    <div class="table-responsive">
		<table class="table table-striped table-responsive table-hover sub-order" width="100%">
			<tr>
				...
				<?php if($approvedStatus && $this->session->userdata('role') == "teacher"){?>
				<th><input type="checkbox" name="selectall" id="selectAll" class="all" value="0"><label for="selectAll">Select All</label></th>
			<?php } ?>
			</tr>
			...
			<tr style=" border-bottom:2px solid #fff">
				...
				<td><?php echo date_display($row->start_date, "m/d/Y");?> @ <?php echo time_display($row->start_date, true);?> to <?php echo time_display($row->end_date, true);?> with <?php echo $row->teacher.', Grade - '.$row->grade_name; ?>, Topic -<?php echo $row->topic_name; ?>
