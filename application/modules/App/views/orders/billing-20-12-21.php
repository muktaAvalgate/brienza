		...
		<table class="table table-striped table-responsive table-hover sub-order" width="100%">
			...
			<?php foreach ($row->order_log as $k1 => $order_log) {?>
				...
				<tr>
					...
					<td style="padding-left: 10px;">
						<?php
						if($this->session->userdata('role') == "teacher" && $row->status != 'Invoice created' && $order_log->new_status == 'Log sent - awaiting principal signature'){
						?>
							<input type="file" id="photo<?php echo $order_log->id;?>" name="attachment_drft" class="upload_file"/>
						<?php } ?>
					</td>
	...
	jQuery(document).ready(function() {
		...
		jQuery('input[type=file]').change(function(){
			...
			if(noCount >20 || fSize > 12582912){
				alert('Cant upload more than 20 files or the maximum file size should be less than 12MB');
