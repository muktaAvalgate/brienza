                ...
                ?>
					...
					<?php if($row->status == "Hours scheduled" && $this->session->userdata('role') == 'administrator') {?>
						<div id="draft_attached_<?php echo $row->id?>" style="display:none;">
							<input type="file" name="attachment[<?php echo $row->id;?>]" class="upload_file" style="width: 209px;font-size: 13px;" onchange="checkFileType(this)">
					...
					<?php if($row->status == "Confirm hours" && $this->session->userdata('role') == 'teacher') {?>
						<div id="create_log_<?php echo $row->id?>" style="display:none">
							...
							<?php }?>
							<input type="file" name="attachment[<?php echo $row->id;?>]" class="upload_file" onchange="checkFileType(this)">
					...
					<?php if($row->status == "Log sent - awaiting principal signature" && $this->session->userdata('role') == 'teacher') {?>
                        <div id="print_log">
                            ...
                            <div class="fileUpload btn btn-sm btn-primary" style=" padding: 2px 10px;margin-top: 3px;">
								...
								<input id="photo_log<?php echo $row->order_schedule_status_id;?>" type="file" class="upload" onchange="upload_document_for_presenter('<?php echo $row->order_schedule_status_id;?>','<?php echo $row->id;?>',this);"/>
                        	</div>
                        </div>
                    <?php }?>
					...
					<?php if((($row->status == "Approved" || $row->status == "Draft attached") && $this->session->userdata('role') == 'teacher') || (($this->session->userdata('role') == 'teacher' || $this->session->userdata('role') == 'administrator') && $row->status == "Log sent - awaiting principal signature") ) {
						if ($row->attachment) {
							echo ' <a href="https://img249.managed.center/'.$row->attachment.'"><img src="'.base_url('assets/dist/images/attachment-icon.png').'" border="0" alt="" title="Click here to download attachment"></a>';
				...
				<tr>
					...
					<?php
						...
						<div class="fileUpload btn btn-sm btn-primary" style=" width: 122px; height: 39px;">
							...
							<input id="photo<?php echo $order_log->id;?>" type="file" class="upload" onchange="reUpload_document_for_admin('<?php echo $order_log->id;?>','<?php echo $row->id;?>',this);"/>
						</div>
						<?php
					...
					<?php
						...
						<?php if ($row->status == 'Log sent - awaiting principal signature' && $order_log->new_status == 'Log sent - awaiting principal signature') { ?>
							<div class="fileUpload btn btn-sm btn-primary" style=" width: 122px; height: 39px;">
							<span> Replace document</span>
							<input id="photo<?php echo $order_log->id;?>" type="file" class="upload" onchange="reUpload_document_for_presenter('<?php echo $order_log->id;?>','<?php echo $row->id;?>',this);"/>
						</div>
...
<script>
	...
	jQuery(document).ready(function() {
		<?php if (empty($order)) { ?>
            setTimeout(function() {   
                alert('There are no logs awaiting principal signature.');
                }, 1000);
        <?php } ?>
...
<script type="text/javascript">
	...
	function reUpload_document_for_admin(log_id,row_id,input){
