                    ...
                    <?php foreach ($pdfmerge_data as $row) { ?> 
                         <div class="row">
                            ...
                            <table class="table table-bordered" id="datatable-order" style="width: 100%;" width="100%">
                                ...
                                <thead>
                                <?php
                                    if(!empty($row->order_id)  && $lastOrderId != $row->order_id){ 
                                        
                                        $loop=0;$lastAttachmentName = ''; ?>
                                    <tr role="row">
                                        ...
                                        <th><?php if($row->is_merged==null){ 
                                                if($attachmentCount[$row->order_id] == 0){
                                            ?>
                                             <button type="button" id="mergeBtn.<?php echo $row->order_id; ?>" class="btn subbtn" data-toggle="modal" data-target="#signModal" onclick="mergePdf('<?php echo $row->order_id; ?>','<?php echo $row->billing_id; ?>')" >Merge</button>
                                             <?php } else{?>
                                ...
                                <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <?php if(!empty($row->school_pdf)){ ?>
                                                <td><?php echo $row->school_pdf; ?></td>
                                            <?php } ?>
                                        <?php if($row->is_merged==null){ ?>
                                              <?php  if($row->pdf_merge_status == 1){
                                            ?>
                                            <td><button type="button" id="compressBtn.<?php echo $row->id; ?>"  class="btn subbtn " data-toggle="modal" data-target="#signModal" onclick="compressPdf('<?php echo $row->id; ?>','<?php echo $row->attachment; ?>','<?php echo $row->order_id; ?>','<?php echo $attachmentCount[$row->order_id]; ?>')" disabled>Ready To Merge</button></td>
                                            <?php } else { ?>
                                                <td><button type="button" id="compressBtn.<?php echo $row->id; ?>"  class="btn subbtn " data-toggle="modal" data-target="#signModal" onclick="compressPdf('<?php echo $row->id; ?>','<?php echo $row->attachment; ?>','<?php echo $row->order_id; ?>','<?php echo $attachmentCount[$row->order_id]; ?>')">Convert To Merge</button></td>
                                            <?php } ?>
                                        </tr>
                                    <?php 
                                        }
