                          ...
                          <td style="padding-left: 10px;">
                              ...
                              if($order_log->new_status != 'Confirm hours'){
                                if ($scheduled->status == 'Log sent - awaiting principal signature' && $order_log->new_status == 'Log sent - awaiting principal signature') { ?>
                                    ...
                                    <input id="photo<?php echo $order_log->id;?>" type="file" class="upload" onchange="reUpload_document_for_presenter('<?php echo $order_log->id;?>','<?php echo $scheduled->id;?>',this);"/>
                                </div>
                            <?php	}else if ($scheduled->status == 'Create invoice' && $order_log->new_status == 'Create invoice'){ ?>
                                <div class="fileUpload btn btn-sm btn-primary" style=" width: 122px; height: 39px;">
                                    <span> Replace document</span>
                                    <input id="photo<?php echo $order_log->id;?>" type="file" class="upload" onchange="reUpload_document_for_presenter('<?php echo $order_log->id;?>','<?php echo $scheduled->id;?>',this);"/>
                                </div>
