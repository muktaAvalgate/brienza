              ...
              ?>
                     ...
                     <?php if($order_log->new_status != 'Create invoice'){ ?>
                          ...
                          <td style="padding-left: 10px;">
                              ...
                              <?php
                              if($order_log->new_status != 'Confirm hours'){
                                      if($order_log->new_status == 'Awaiting Review' && $scheduled->status != 'Invoice created'){ ?>
                          ...
                          <td>
                              ...
                              if($order_log->new_status != 'Confirm hours'){
                                      if($order_log->new_status == 'Awaiting Review' && $scheduled->status != 'Invoice created'){ ?>
                                          <button type="button" onclick="reUpload_document_for_presenter('<?php echo $order_log->id;?>','<?php echo $scheduled->id;?>')" class="btn btn-primary"><span class="glyphicon"></span> Reupload</button>
                                     <?php }
                                  }
                              ?>
                          </td>
