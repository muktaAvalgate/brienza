...
<div class="container-fluid main">
	...
	<div class="row">
		<div class="col-md-3"> 
			<div class="list-group">
			  ...
			  <?php if($this->session->userdata('role') == 'administrator') {?>
              <a href="<?php echo base_url('app/notifications/?folder=logs');?>" class="list-group-item <?php if($folder=="logs") {echo "active";}?>">Logs <?php ?><span class="badge" style="color: #fff; background-color: #d9534f;"><?php echo $log_details;?></span><?php ?></a>
              <?php }?>
				...
				<table class="table table-responsive" width="100%">
					<tbody>
						...
						<?php 
                            ...
                            $summery = "<span>".$item->subject."</span> - ".strip_tags($item->description); $summery = character_limiter($summery, 85);}
                            else{
                                $summery = "<span>".'Message'."</span> - ".strip_tags($item->description); $summery = character_limiter($summery, 85);
                            }
                        ?>
		...
		<div class="col-md-9">
            ...
            <?php //if($this->session->userdata('role') == 'administrator') {?>
            <?php if($folder=="logs" && $this->session->userdata('role') == 'administrator'){ ?>
                <div class="" id="notification_list">
                    <!-- Table -->
                    <table class="table table-responsive" width="100%">
                        <tbody>
                            ...
                            <?php //echo'<pre>';print_r($unread_logs);die; ?>

                        <?php if ($unread_logs == false) { ?>
                        <tr>
                            <td colspan="100%">Your list is empty.</td>
                        </tr>
                        <?php } ?>
                            
                        <?php if($unread_logs != false){ foreach($unread_logs as $logs) { //echo'<pre>';print_r($logs);die; ?>
                            ...
                            ?>
                                ...
                                <!-- <td><?php echo date_display($row->start_date, "m/d/Y");?> @ <?php echo time_display($row->start_date, true);?>-<?php echo time_display($row->end_date, true);?> with <?php echo $row->teacher;?> </td> -->
                                    ...
                                    <td> <span style="font-weight:bold; color:#4d4e4f;"><?php echo date_display($logs->start_date, "m/d/Y"); ?> @ <?php echo time_display($logs->start_date, true);?> - <?php echo time_display($logs->end_date, true);?></span> </td>

									<td> <span style="font-weight:bold; color:#4d4e4f;"><?php echo $logs->name ?></span> </td>

                                    <td> <span style="font-weight:bold; color:#4d4e4f; margin-left: 4rem;"><?php echo $logs->total_hours ?></span> </td>
                                    <td> <span style="font-weight:bold; color:#4d4e4f;"><?php echo $logs->order_no ?></span> </td>
                                    <!-- <td> <?php echo $pdf ?> </td> -->
                                    <td ><a href="<?php echo base_url('app/notifications/log_download/'.$logs->order_schedule_id); ?>"><img src="<?php echo base_url('assets/images/pdf_download.png'); ?>" style="border-radius:1% !important; border: 1px solid #ffffff; !important; width:65px !important; height:40px !important;" alt="" title="Click here to download log"></a></td>
                                </tr>
                            <?php }else{ ?>
                                    <tr>
                                        <td> <?php echo date_display($logs->start_date, "m/d/Y"); ?> @ <?php echo time_display($logs->start_date, true);?> - <?php echo time_display($logs->end_date, true);?></td>

										<td> <span style=""><?php echo $logs->name; ?></span> </td>

                                        <td> <span style="margin-left: 4rem;"><?php echo $logs->total_hours ?></span> </td>
                                        <td> <span><?php echo $logs->order_no ?></span> </td>
                                        <!-- <td><a href="<?php echo base_url('app/notifications/log_download/'.$logs->order_schedule_id); ?>"><img src="<?php echo base_url('assets/dist/images/attachment-icon.png'); ?>" border="0" alt="" title="Click here to download log"></a></td> -->
                                        <td ><a href="<?php echo base_url('app/notifications/log_download/'.$logs->order_schedule_id); ?>"><img src="<?php echo base_url('assets/images/pdf_download.png'); ?>" style="border-radius:1% !important; border: 1px solid #ffffff; !important; width:65px !important; height:40px !important;" alt="" title="Click here to download log"></a></td>
                                    </tr>
                ...
                <div style="margin-left:-34%; margin-top:5rem;"><?php echo $this->pagination->create_links(); ?></div>
                <?php }else{ ?>
	...
	<?php if($folder!="logs"){ echo $this->pagination->create_links();} ?>
</div>
