<?php
$coordinator_id = $this->uri->segment(4);
?>
<div class="subnav">
	<div class="container-fluid">
		...
		<div id="sub-menu" class="pull-right">
        	<ul class="nav nav-pills">
				<li class="active"><?php //echo render_link('order', '<span class="glyphicon glyphicon-shopping-cart"></span> Orders');?>
					
				<a href="<?php echo base_url('app/coordinator/order/'.$coordinator_id) ?>" title="Orders"><span class="glyphicon glyphicon-shopping-cart"></span> Orders</a>
				</li>
				<?php if($this->session->userdata('role') != 'school_admin'){ ?>
					<li><?php //echo render_link('order_add', '<span class="glyphicon glyphicon-plus-sign"></span> Create New Order');?>
					<?php $addURL = (isset($coordinator_id) && $coordinator_id != '') ? '?id='.$coordinator_id : (($this->session->userdata('role') == 'coordinator') ? '?ref=order' : ''); ?>
					<a href="<?php echo base_url('app/coordinator/order_add/'.$addURL) ?>" title="Create Order"><span class="glyphicon glyphicon-plus-sign"></span> Create New Order</a>
...
<div class="container-fluid main">
	...
	<?php
		...
		echo form_open(base_url('app/coordinator/co_search_submit/'.$coID), $attributes);
	?>
	...
	<div class="table-responsive">
	    ...
	    <table class="table table-striped table-responsive table-hover" width="100%">
	    	<thead>
	    		<tr>
	            	<?php if($this->session->userdata('role') == 'school_admin' || $this->session->userdata('role') == 'coordinator'){ ?>
