				...
				<div class="form-group">
		  		...
		  		<div class="col-sm-7">
						<select name="meta[rate_type]" class="form-control" required>
							...
							<option value="percentage" <?php if(!empty($teacher->meta['rate_type']) && strtolower($teacher->meta['rate_type']) == 'percentage') echo 'selected'; ?>>Percentage</option>
							<option value="fixed" <?php if(!empty($teacher->meta['rate_type']) && strtolower($teacher->meta['rate_type']) == 'fixed') echo 'selected'; ?>>Fixed</option>
						</select>
		  			<div class="help-block with-errors"></div>
		  		</div>
		  	</div>					
