     		...
     		if ($this->form_validation->run())
     		{
    			...
    			if (!empty($pic['name'])) {
					...
					if (!$this->upload->do_upload('profile_pic')) {
						...
						$fileType_parts = explode('/', $fileType);
						if(($fileType_parts[1]=="jpg" || $fileType_parts[1]=="png" || $fileType_parts[1]=="jpeg" || $fileType_parts[1]=="bmp") && strpos($this->upload->display_errors() ,  "filetype")){
								$this->session->set_flashdata('message_type', 'danger');
								$this->session->set_flashdata('message', '<strong>Oh snap!</strong> The selected profile picture is corrupted. Please select a different image.');
								redirect('profile');
						}else{
							$this->session->set_flashdata('message_type', 'danger');
