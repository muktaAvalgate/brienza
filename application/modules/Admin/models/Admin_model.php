...
class Admin_model extends CI_Model {
	...
	public function get_users_list($filter = array(), $order = null, $dir = null, $count = false) {
		...
		if (isset($filter['schl_status_fr_assign_titles']) && ($filter['schl_status_fr_assign_titles'] == 'active')) {
            $this->db->where('users.status', $filter['assign_titles']);
        }
		...
		if (isset($filter['email']) && $filter['email'] !== "") {
            $this->db->where('users.id', $filter['email']);
        }

		if (isset($filter['school']) && $filter['school'] !== "") {
            $this->db->where('users.id', $filter['school']);
        }

        if (isset($filter['contact_person']) && $filter['contact_person'] !== "") {
            $this->db->where('users.id', $filter['contact_person']);
        }

		// adding 18-04-2023
		if (isset($filter['search_box']) && $filter['search_box'] != '') {
    		// $this->db->like('users.email', $search_box);
			$this->db->group_start();  //group start
			$this->db->like('users.email', $filter['search_box']);
			$this->db->or_like('users.first_name',$filter['search_box']); 
	...
	public function get_users_list_new($filter = array(), $order = null, $dir = null, $count = false) {
		// echo '<pre>'; print_r($filter); 
		$this->db->select('users.id, users.role_id, users.first_name, users.last_name, users.email, users.last_login, users.created_on, users.status, users.headerImg, roles.role_name,CONCAT(users.first_name, " ", users.last_name) as presenter');
		$this->db->from('users');
		$this->db->join('roles', 'users.role_id = roles.id');
		
		if (isset($filter['search_box_schl']) && $filter['search_box_schl'] != '') {
		...
		// adding 17-04-2023
		if (isset($filter['search_box_schl']) && $filter['search_box_schl'] != '') {
    		// $this->db->like('users.email', $search_box_schl);
			$this->db->group_start();  //group start
			$this->db->like('users.email', $filter['search_box_schl']);
			$this->db->or_like('users.first_name',$filter['search_box_schl']); 
			$this->db->or_like('users.last_name',$filter['search_box_schl']); 
			$this->db->or_like('user_meta.meta_value',$filter['search_box_schl']); 
			$this->db->group_end();  //group ed
   		}
...
//echo "<pre>";print_r($query->result());die;
	...
	public function get_coordinator_presenter_list_unique($coordinator_id)
    {
        $this->db->select('coordinator_presentator_school.*, users.first_name, users.last_name,users.email');
        // $this->db->distinct('coordinator_presentator_school.presenter_id');
        $this->db->from('coordinator_presentator_school');
        $this->db->join('users', 'coordinator_presentator_school.presenter_id = users.id');
        $this->db->where('coordinator_presentator_school.coordinator_id', $coordinator_id);
        $this->db->where('coordinator_presentator_school.is_deleted', '0');
        $this->db->group_by('coordinator_presentator_school.presenter_id');
	...
	function update_user_meta_specific($user_id, $meta = array()) {
	...
	public function get_duplicate_mail($email)
	{
		$this->db->select('*');
		$this->db->from('users');
	...
	public function get_schools_list_search()
    {
        $this->db->select('users.*');
        $this->db->from('users');
        $this->db->where('users.is_deleted',0);
	...
	public function exists_profile_pic_key($id){
		$this->db->select('*');
		$this->db->from('user_meta');
