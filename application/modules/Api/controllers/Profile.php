...
class Profile extends REST_Controller {
            ...
            if($user_details == FALSE){
                ...
                foreach($user_meta as $k => $v){
                    ...
                    if($v->meta_key == 'profile_pic'){
                        if(!empty($v->meta_key)){
                            // $final_meta_array[$v->meta_key]=$v->meta_value;
                            $final_meta_array['profile_pic'] = BASE_URL.DIR_PROFILE_PICTURE_THUMB.$v->meta_value;
                        }else{
                ...
                if($user_details->role_name == 'Teacher'){
                    $final_meta_array['role']='Presenter';
                }else{
                    $final_meta_array['role']=$user_details->role_name;
                }
    ...
    public function forgetPassword_post(){
        ...
        if($this->form_validation->run() === FALSE){
            ...
            if ($info = $this->Api_model->is_valid_data('users', array('email' => $email, 'role_id' => '3'))) {
                $user_id = $info->id;
