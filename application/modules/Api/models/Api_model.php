...
class Api_model extends CI_Model{
	...
	public function get_order_list_order($filter = array(), $order = null, $dir = null, $count = false) 
    {
        ...
        // if($this->session->userdata('role') == 'teacher'){
            ...
            $appendCondForPersenter = ' LEFT JOIN order_assigned_presenters ON order_schedules.presenter_id="'.$filter['presenter'].'" AND orders.id=order_assigned_presenters.order_id';
            $scheduleByPresenter = '  AND order_schedules.created_by = "'.$filter['presenter'].'"';
        // }else{
        //     $selectQuery = '';
        ...
        // if($this->session->userdata('role') == 'teacher'){
            $this->db->join('order_assigned_presenters', 'orders.id = order_assigned_presenters.order_id AND order_assigned_presenters.presenter_id="'.$filter['presenter'].'"', 'left');
        // }
    ...
    function get_order_schedule_for_api($order_id=null, $presenter_id=null, $group_by='') {
        ...
        foreach ($record as $k => $v) {
            ...
            // principal name, content txt
            $this->db->select('order_schedule_status_log.content_txt,order_schedule_status_log.principal_name');
            $this->db->from('order_schedule_status_log');
            $this->db->where('order_schedule_status_log.order_schedule_id', $v->id);
            $this->db->where('order_schedule_status_log.new_status', 'Log sent - awaiting principal signature');
            $this->db->where('order_schedule_status_log.old_status', 'Create log');
            $this->db->limit(20);
            $q1 = $this->db->get();
            if ($q1->num_rows() > 0) {
                $res1 = $q1->row();
                $record[$k]->content_text = $res1->content_txt;
                $record[$k]->principal_name = $res1->principal_name;
            }else{
                $record[$k]->content_text = '';
                $record[$k]->principal_name = '';
            }
            // $res1 = $q1->row();
            // $record[$k]->content_text = $res1->content_txt;
            // $record[$k]->principal_name = $res1->principal_name;
            // end principal name, content txt

            // sign
            $this->db->select('order_schedule_status_log.content');
            $this->db->from('order_schedule_status_log');
            $this->db->where('order_schedule_status_log.order_schedule_id', $v->id);
            // $this->db->where('order_schedule_status_log.new_status', 'Create invoice');

            $where = '(order_schedule_status_log.new_status="Create invoice" or order_schedule_status_log.new_status = "Awaiting Review")';
            $this->db->where($where);
            ...
            // $record[$k]->sign = $res2->content;
            // end sign
            if (date('Y-m-d H:i:s') > $v->end_date){
                $confirm_btn_status = 1;
            }else{
                $confirm_btn_status = 0;
            }
            $record[$k]->confirm_btn_status = $confirm_btn_status;

        }
        return $record;
	}

    public function get_teacher_orders_horstoconfirm($presenter_id){
        // Total "Hours to confirm" new implementation 17-09-2021
