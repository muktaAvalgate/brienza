    ...
    */
	...
	public function get_dashboard()
	{
		try {
			...
			$this->db->select('order_assigned_presenters.presenter_id,order_assigned_presenters.order_id');
			$this->db->from('order_assigned_presenters');

			$this->db->join('orders', 'order_assigned_presenters.order_id = orders.id'); // 19-06-2023
			$this->db->where('orders.is_deleted', 0); // 19-06-2023
	...
	public function get_agenda_schedule($filter = array(), $order = null, $dir = null, $count = false,$curr_session_id){
		...
		*/
		...
		if (isset($filter['statusSort']) && $filter['statusSort'] !== "") {
			$this->db->order_by('order_schedules.status',$filter['statusSort']);
			$this->db->order_by('order_schedules.start_date DESC');
		}else if(isset($filter['sessionSort']) && $filter['sessionSort'] !== ""){
			$this->db->order_by('order_schedules.start_date',$filter['sessionSort']);
		}else{
			$this->db->order_by('order_schedules.status');
			$this->db->order_by('order_schedules.start_date DESC');
